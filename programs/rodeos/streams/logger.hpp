...
class logger : public stream_handler {
   std::vector<dcd::name> routes_;

 public:
   logger(std::vector<dcd::name> routes) : routes_(std::move(routes)) {
      ilog("logger initialized");
   }

   const std::vector<dcd::name>& get_routes() const override { return routes_; }

   void publish(const char* data, uint64_t data_size, const dcd::name& routing_key) override {
...
inline void initialize_loggers(std::vector<std::unique_ptr<stream_handler>>& streams,
   ...
   for (const auto& routes_str : loggers) {
      std::vector<dcd::name> routes = extract_routes(routes_str);
