...
int main(int argc, char** argv) {
   using namespace dcd::dcdnode_sectl::config;
   ...
   if(!dcd::secure_enclave::application_signed()) {
      std::cerr << node_executable_name <<  "-sectl is not signed so it is unable to managed Secure Enclave keys" << std::endl;
      return 1;
   }
   if(!dcd::secure_enclave::hardware_supports_secure_enclave()) {
      std::cerr << "This device does not appear to have a Secure Enclave" << std::endl;
      return 1;
   }

   if(create)
      std::cout << dcd::secure_enclave::create_key().public_key().to_string() << std::endl;

   std::set<dcd::secure_enclave::secure_enclave_key> allkeys = dcd::secure_enclave::get_all_keys();

   if(key_to_delete != fc::crypto::public_key()) {
      auto it = allkeys.begin();
      for(; it != allkeys.end(); ++it) {
         if(it->public_key() == key_to_delete) {
            dcd::secure_enclave::delete_key(std::move(allkeys.extract(it).value()));
            break;
         }
      }
      if(it == allkeys.end()) {
