...
//copy pasta from dcdksd's main.cpp
...
void ensure_dcdksd_running(CLI::App* app) {
    if (no_auto_dcdksd)
        return;
    // get, version, net, convert do not require dcdksd
    ...
    // This extra check is necessary when running dcdcli like this: ./dcdcli ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if dcdcli and dcdksd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("dcdksd").append(key_store_executable_name);
    }
...
/// COMMENT
      if( res.refund_request.is_object() ) {
         ...
         //fc::time_point refund_time = request_time + fc::days(3); /// Temporarily reduced refund delay for testnet
         fc::time_point refund_time = request_time + fc::hours(2); 
...
int main( int argc, char** argv ) {
   ...
   app.add_flag( "--no-auto-" + string(key_store_executable_name), no_auto_dcdksd, localized("Don't automatically launch a ${k} if one is not currently running", ("k", key_store_executable_name)));
   app.parse_complete_callback([&app]{ ensure_dcdksd_running(&app);});
...
//                                ->check(CLI::ExistingFile);
   ...
   auto stopdcdksd = wallet->add_subcommand("stop", localized("Stop ${k}.", ("k", key_store_executable_name)));
   stopdcdksd->callback([] {
      const auto& v = call(wallet_url, dcdksd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success dcdksd responds with empty object
