...
using dcd::check;
using dcd::convert_to_bin;
using dcd::chain::block_state_ptr;
using dcd::chain::builtin_protocol_feature_t;
using dcd::chain::digest_type;
using dcd::chain::kv_bad_db_id;
using dcd::chain::kv_bad_iter;
using dcd::chain::kv_context;
using dcd::chain::kvram_id;
using dcd::chain::protocol_feature_exception;
using dcd::chain::protocol_feature_set;
using dcd::chain::signed_transaction;
using dcd::chain::transaction_trace_ptr;
using dcd::chain::packed_transaction_ptr;
using dcd::state_history::block_position;
using dcd::state_history::create_deltas;
using dcd::state_history::get_blocks_result_v1;
using dcd::state_history::state_result;
using dcd::vm::span;

struct callbacks;
using rhf_t     = dcd::vm::registered_host_functions<callbacks>;
...
// Handle dcd version differences
namespace {
template <typename T>
auto to_uint64_t(T n) -> std::enable_if_t<std::is_same_v<T, dcd::chain::name>, decltype(n.value)> {
   return n.value;
}
template <typename T>
auto to_uint64_t(T n) -> std::enable_if_t<std::is_same_v<T, dcd::chain::name>, decltype(n.to_uint64_t())> {
   return n.to_uint64_t();
}

template <typename C, typename F0, typename F1, typename G>
auto do_startup(C&& control, F0&&, F1&& f1, G &&)
      -> std::enable_if_t<std::is_constructible_v<std::decay_t<decltype(*control)>, dcd::chain::controller::config,
...
struct __attribute__((__may_alias__)) xxx_transaction_checktime_timer {
   ...
   dcd::chain::platform_timer& _timer;
};
struct transaction_checktime_factory {
   dcd::chain::platform_timer              timer;
   std::atomic_bool                          expired;
   dcd::chain::transaction_checktime_timer get() {
      xxx_transaction_checktime_timer result{ expired, timer };
      return std::move(*reinterpret_cast<dcd::chain::transaction_checktime_timer*>(&result));
   }
};
}; // namespace

struct intrinsic_context {
   dcd::chain::controller&                          control;
   dcd::chain::packed_transaction                   trx;
   std::unique_ptr<dcd::chain::transaction_context> trx_ctx;
   std::unique_ptr<dcd::chain::apply_context>       apply_context;

   intrinsic_context(dcd::chain::controller& control) : control{ control } {
      static transaction_checktime_factory xxx_timer;

      dcd::chain::signed_transaction strx;
      strx.actions.emplace_back();
      strx.actions.back().account = dcd::chain::name{ "dcd.null" };
      strx.actions.back().authorization.push_back({ dcd::chain::name{ "dcd" }, dcd::chain::name{ "active" } });
      trx = dcd::chain::packed_transaction( std::move(strx), true );
      trx_ctx = std::make_unique<dcd::chain::transaction_context>(control, trx, xxx_timer.get(),
                                                                    fc::time_point::now());
      trx_ctx->init_for_implicit_trx(0);
      trx_ctx->exec();
      apply_context = std::make_unique<dcd::chain::apply_context>(control, *trx_ctx, 1, 0);
...
protocol_feature_set make_protocol_feature_set() {
   ...
   for (const auto& p : dcd::chain::builtin_protocol_feature_codenames) { add_builtins(p.first); }

   return pfs;
}

template <typename T>
using wasm_ptr = dcd::vm::argument_proxy<T*>;
...
struct test_chain {
   dcd::chain::private_key_type producer_key{ "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"s };

   fc::temp_directory                                dir;
   std::unique_ptr<dcd::chain::controller::config> cfg;
   std::unique_ptr<dcd::chain::controller>         control;
   std::optional<scoped_connection>                  applied_transaction_connection;
   std::optional<scoped_connection>                  accepted_block_connection;
   dcd::state_history::transaction_trace_cache     trace_cache;
   ...
   test_chain(const char* snapshot) {
      ...
      if (snapshot && *snapshot) {
         std::optional<dcd::chain::chain_id_type> chain_id;
         {
            std::ifstream temp_file(snapshot, (std::ios::in | std::ios::binary));
            if (!temp_file.is_open())
               throw std::runtime_error("can not open " + std::string{ snapshot });
            dcd::chain::istream_snapshot_reader tmp_reader(temp_file);
            tmp_reader.validate();
            chain_id = dcd::chain::controller::extract_chain_id(tmp_reader);
         }
         snapshot_file.emplace(snapshot, std::ios::in | std::ios::binary);
         snapshot_reader = std::make_shared<dcd::chain::istream_snapshot_reader>(*snapshot_file);
         control         = std::make_unique<dcd::chain::controller>(*cfg, make_protocol_feature_set(), *chain_id);
      } else {
         control = std::make_unique<dcd::chain::controller>(*cfg, make_protocol_feature_set(),
      ...
      if (snapshot_reader) {
         ...
         control->start_block(control->head_block_time() + fc::microseconds(block_interval_us), 0,
                              { *control->get_protocol_feature_manager().get_builtin_digest(
                                    dcd::chain::builtin_protocol_feature_t::preactivate_feature) });
      }
   }
   ...
   void finish_block() {
      ...
      control->finalize_block([&](dcd::chain::digest_type d) { return std::vector{ producer_key.sign(d) }; });
...
dcd::checksum256 convert(const dcd::chain::checksum_type& obj) {
   std::array<uint8_t, 32> bytes;
   static_assert(bytes.size() == sizeof(obj));
   memcpy(bytes.data(), &obj, bytes.size());
   return dcd::checksum256(bytes);
}

chain_types::account_delta convert(const dcd::chain::account_delta& obj) {
...
chain_types::action_receipt_v0 convert(const dcd::chain::action_receipt& obj) {
   ...
   for (auto& auth : obj.auth_sequence)
      result.auth_sequence.push_back({ dcd::name{ to_uint64_t(auth.first) }, auth.second });
   result.code_sequence.value = obj.code_sequence.value;
   result.abi_sequence.value  = obj.abi_sequence.value;
   return result;
}

chain_types::action convert(const dcd::chain::action& obj) {
   ...
   for (auto& auth : obj.authorization)
      result.authorization.push_back(
            { dcd::name{ to_uint64_t(auth.actor) }, dcd::name{ to_uint64_t(auth.permission) } });
   result.data = { obj.data.data(), obj.data.data() + obj.data.size() };
   return result;
}

chain_types::action_trace_v1 convert(const dcd::chain::action_trace& obj) {
...
chain_types::transaction_trace_v0 convert(const dcd::chain::transaction_trace& obj) {
...
DCD_REFLECT(contract_row, block_num, present, code, scope, table, primary_key, payer, value);
...
struct push_trx_args {
   dcd::chain::bytes                         transaction;
   std::vector<dcd::chain::bytes>            context_free_data;
   std::vector<dcd::chain::signature_type>   signatures;
   std::vector<dcd::chain::private_key_type> keys;
};
...
struct callbacks {
   ...
   void check_bounds(void* data, size_t size) {
      ...
      dcd::vm::ignore_unused_variable_warning(check);
   }
   ...
   template <typename T>
   T unpack(const dcd::input_stream& data) {
   ...
   char* alloc(uint32_t cb_alloc_data, uint32_t cb_alloc, uint32_t size) {
      ...
      auto result = state.backend.get_context().execute( //
            this, dcd::vm::jit_visitor(42), state.backend.get_module().tables[0].table[cb_alloc], cb_alloc_data,
            size);
      if (!result || !result->is_a<dcd::vm::i32_const_t>())
   ...
   void dcd_assert_message(bool condition, span<const char> msg) {
      if (!condition)
   ...
   void replace_producer_keys(uint32_t chain_index, span<const char> key) {
      ...
      auto  k     = unpack<dcd::chain::public_key_type>(key);
   ...
   void replace_account_keys(uint32_t chain_index, uint64_t account, uint64_t permission, span<const char> key) {
      auto& chain = assert_chain(chain_index);
      auto  k     = unpack<dcd::chain::public_key_type>(key);
      chain.control->replace_account_keys(dcd::chain::name{ account }, dcd::chain::name{ permission }, k);
   }
   ...
   void push_transaction(uint32_t chain_index, span<const char> args_packed, uint32_t cb_alloc_data,
      ...
      auto                             transaction = unpack<dcd::chain::transaction>(args.transaction);
      dcd::chain::signed_transaction signed_trx{ std::move(transaction), std::move(args.signatures),
      ...
      auto ptrx = std::make_shared<dcd::chain::packed_transaction>(
            std::move(signed_trx), true, dcd::chain::packed_transaction::compression_type::none);
      auto fut = dcd::chain::transaction_metadata::start_recover_keys(
   ...
   bool exec_deferred(uint32_t chain_index, uint32_t cb_alloc_data, uint32_t cb_alloc) {
      ...
      chain.start_if_needed();
      const auto& idx =
            chain.control->db().get_index<dcd::chain::generated_transaction_multi_index, dcd::chain::by_delay>();
      auto itr = idx.begin();
   ...
   void roddcd_push_transaction(uint32_t roddcd, span<const char> packed_args, uint32_t cb_alloc_data,
      ...
      dcd::chain::signed_transaction signed_trx{ std::move(transaction), std::move(args.signatures),
                                                   std::move(args.context_free_data) };
      for (auto& key : args.keys) signed_trx.sign(key, chain.control->get_chain_id());
      dcd::chain::packed_transaction ptrx{ std::move(signed_trx), true, dcd::chain::packed_transaction::compression_type::none };
...
void register_callbacks() {
   ...
   rhf_t::add<&callbacks::dcd_assert_message>("env", "dcd_assert_message");
...
static void run(const char* wasm, const std::vector<std::string>& args) {
   dcd::vm::wasm_allocator wa;
   auto                      code = dcd::vm::read_wasm(wasm);
   backend_t                 backend(code, nullptr);
   ::state                   state{ wasm, wa, backend, dcd::convert_to_bin(args) };
   callbacks                 cb{ state };
...
const char usage[] = "usage: dcd-tester [-h or --help] [-v or --verbose] file.wasm [args for wasm]\n";
