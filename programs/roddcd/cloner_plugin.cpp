...
using roddcd::roddcd_db_partition;
using roddcd::roddcd_db_snapshot;
using roddcd::roddcd_filter;
...
struct cloner_session : ship_client::connection_callbacks, std::enable_shared_from_this<cloner_session> {
   ...
   std::shared_ptr<roddcd_db_partition> partition =
         std::make_shared<roddcd_db_partition>(db, std::vector<char>{}); // todo: prefix

   std::optional<roddcd_db_snapshot>        roddcd_snapshot;
   std::shared_ptr<ship_client::connection> connection;
   bool                                     reported_block = false;
   std::unique_ptr<roddcd_filter>           filter         = {}; // todo: remove

   cloner_session(cloner_plugin_impl* my) : my(my), config(my->config) {
      // todo: remove
      if (!config->filter_wasm.empty())
         filter = std::make_unique<roddcd_filter>(config->filter_name, config->filter_wasm);
   }

   void connect(asio::io_context& ioc) {
      roddcd_snapshot.emplace(partition, true);

      ilog("cloner database status:");
      ilog("    revisions:    ${f} - ${r}",
           ("f", roddcd_snapshot->undo_stack->first_revision())("r", roddcd_snapshot->undo_stack->revision()));
      ilog("    chain:        ${a}", ("a", dcd::convert_to_json(roddcd_snapshot->chain_id)));
      ilog("    head:         ${a} ${b}",
           ("a", roddcd_snapshot->head)("b", dcd::convert_to_json(roddcd_snapshot->head_id)));
      ilog("    irreversible: ${a} ${b}",
           ("a", roddcd_snapshot->irreversible)(
                 "b", dcd::convert_to_json(roddcd_snapshot->irreversible_id)));

      roddcd_snapshot->end_write(true);
      db->flush(true, true);
   ...
   bool received(get_status_result_v0& status, dcd::input_stream bin) override {
      ...
      if (roddcd_snapshot->chain_id == dcd::checksum256{})
         roddcd_snapshot->chain_id = status.chain_id;
      if (roddcd_snapshot->chain_id != status.chain_id)
         throw std::runtime_error(
               "database is for chain " + dcd::convert_to_json(roddcd_snapshot->chain_id) +
               " but dcdnode has chain " + dcd::convert_to_json(status.chain_id));
      ilog("request blocks");
      connection->request_blocks(status, std::max(config->skip_to, roddcd_snapshot->head + 1), get_positions(),
   ...
   std::vector<block_position> get_positions() {
      ...
      if (roddcd_snapshot->head) {
         roddcd::db_view_state view_state{ roddcd::state_account, *db, *roddcd_snapshot->write_session,
                                           partition->contract_kv_prefix };
         for (uint32_t i = roddcd_snapshot->irreversible; i <= roddcd_snapshot->head; ++i) {
            auto info = roddcd::get_state_row<roddcd::block_info>(
            ...
            auto& info0 = std::get<roddcd::block_info_v0>(info->second);
   ...
   bool process_received(Get_Blocks_Result& result, dcd::input_stream bin) {
      ...
      if (roddcd_snapshot->head && result.this_block->block_num > roddcd_snapshot->head + 1)
         throw std::runtime_error("state-history plugin is missing block " + std::to_string(roddcd_snapshot->head + 1));

      roddcd_snapshot->start_block(result);
      if (result.this_block->block_num <= roddcd_snapshot->head)
      ...
      roddcd_snapshot->write_block_info(result);
      roddcd_snapshot->write_deltas(result, [] { return app().is_quiting(); });

      if (filter) {
         filter->process(*roddcd_snapshot, result, bin, [&](const char* data, uint64_t data_size) {
            if (my->streamer) {
      ...
      roddcd_snapshot->end_block(result, false);
      return true;
   }
...
void cloner_plugin::set_program_options(options_description& cli, options_description& cfg) {
   ...
   op("telemetry-service-name", bpo::value<std::string>()->default_value(b1::roddcd::config::roddcd_executable_name),
