...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance("dcd.token"_n, symbol(SY(4,CUR)), account);
}

const std::string dcd_token = name("dcd.token"_n).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account("dcd.token"_n);
   ...
   chain.push_action("dcd.token"_n, name("issue"), "dcd.token"_n, fc::mutable_variant_object()
           ("to",       dcd_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action("dcd.token"_n, name("transfer"), "dcd.token"_n, fc::mutable_variant_object()
       ("from", dcd_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, "dcd.token"_n);
   ...
   trace = chain.push_action("dcd.token"_n, name("transfer"), "tester"_n, fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, "dcd.token"_n);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, "tester"_n);
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   // transfer from dcd.token to tester
   trace = chain.push_action("dcd.token"_n, name("transfer"), "dcd.token"_n, fc::mutable_variant_object()
       ("from", "dcd.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>("dcd.token"_n);
   ...
   const auto a = chain.control->db().get<account_object,by_name>("dcd.token"_n).get_abi();

   string action_type_name = abis.get_action_type(name("transfer"));

   action act;
   act.account = "dcd.token"_n;
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action("dcd.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, "first"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action("dcd.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, "second"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action("dcd.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
