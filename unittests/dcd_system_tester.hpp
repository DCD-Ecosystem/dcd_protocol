...
namespace dcd_system {

class dcd_system_tester : public TESTER {
public:

   dcd_system_tester()
   : dcd_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   dcd_system_tester(Lambda setup) {
      setup(*this);
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("dcd") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("dcd")) + get_balance(name("dcd.ramfee")) + get_balance(name("dcd.stake")) + get_balance(name("dcd.ram")) );
   }
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( "dcd.msig"_n, config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( name("dcd"), name("dcd.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( "dcd.msig"_n, contracts::dcd_msig_wasm() );
         set_abi( "dcd.msig"_n, contracts::dcd_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( "dcd.msig"_n );
         abi_def msig_abi;
...
inline uint64_t M( const string& dcd_str ) {
   return core_from_string( dcd_str ).get_amount();
}
