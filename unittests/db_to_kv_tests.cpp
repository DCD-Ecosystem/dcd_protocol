...
using key_type = dcd::chain::backing_store::db_key_value_format::key_type;
using name = dcd::chain::name;
...
constexpr uint64_t legacy_overhead_size_without_type =
      dcd::chain::backing_store::db_key_value_format::detail::prefix_size<legacy_key>(); // 8 (scope) + 8 (table)
...
constexpr uint64_t current_overhead_size_without_type =
      dcd::chain::backing_store::db_key_value_format::detail::prefix_size<dcd::session::shared_bytes>(); // 8 (scope) + 8 (table)
...
constexpr dcd::chain::uint128_t create_uint128(uint64_t upper, uint64_t lower) {
   return (dcd::chain::uint128_t(upper) << 64) + lower;
}

constexpr dcd::chain::key256_t create_key256(dcd::chain::uint128_t upper, dcd::chain::uint128_t lower) {
   return { upper, lower };
}
...
struct helper<uint64_t> {
   ...
   using alt_type_t = dcd::chain::uint128_t;
   const type_t init = 0x0;
   type_t inc(type_t t) { return ++t; }
};

template<>
struct helper<dcd::chain::uint128_t> {
   using type_t = dcd::chain::uint128_t;
   using alt_type_t = dcd::chain::key256_t;
   const type_t init = create_uint128(0, 0);
   type_t inc(type_t t) { return ++t; }
};

template<>
struct helper<dcd::chain::key256_t> {
   using type_t = dcd::chain::key256_t;
...
void verify_secondary_wont_convert(const b1::chain_kv::bytes& composite_key, const name exp_scope, const name exp_table) {
   ...
   BOOST_CHECK(!dcd::chain::backing_store::db_key_value_format::get_secondary_key(composite_key, scope, table, key, primary_key));
...
void verify_other_gets(const b1::chain_kv::bytes& composite_key, key_type except, const name exp_scope, const name exp_table) {
   if (except != key_type::primary) {
      ...
      name table;
      uint64_t key = 0;
      BOOST_CHECK(!dcd::chain::backing_store::db_key_value_format::get_primary_key(composite_key, scope, table, key));
      BOOST_CHECK_EQUAL(exp_scope.to_string(), scope.to_string());
      BOOST_CHECK_EQUAL(exp_table.to_string(), table.to_string());
      BOOST_CHECK(0 == key);
   }
   if (except != key_type::sec_i64) { verify_secondary_wont_convert<uint64_t>(composite_key, exp_scope, exp_table); }
   if (except != key_type::sec_i128) { verify_secondary_wont_convert<dcd::chain::uint128_t>(composite_key, exp_scope, exp_table); }
   if (except != key_type::sec_i256) { verify_secondary_wont_convert<dcd::chain::key256_t>(composite_key, exp_scope, exp_table); }
   ...
   if (except != key_type::table) {
      name scope;
      name table;
      BOOST_CHECK(!dcd::chain::backing_store::db_key_value_format::get_table_key(composite_key, scope, table));
      BOOST_CHECK_EQUAL(exp_scope.to_string(), scope.to_string());
      BOOST_CHECK_EQUAL(exp_table.to_string(), table.to_string());
   }
   name scope;
   name table;
   BOOST_CHECK_NO_THROW(dcd::chain::backing_store::db_key_value_format::get_prefix_key(composite_key, scope, table));
   BOOST_CHECK_EQUAL(exp_scope.to_string(), scope.to_string());
   BOOST_CHECK_EQUAL(exp_table.to_string(), table.to_string());
}
...
void verify_scope_table_order(T low_key, T high_key, KeyFunc key_func, bool skip_trailing_prim_key) {
   ...
   std::vector<dcd::session::shared_bytes> full_keys;
...
b1::chain_kv::bytes prim_drop_extra_key(name scope, name table, uint64_t db_key, uint64_t unused_primary_key) {
   return dcd::chain::backing_store::db_key_value_format::create_primary_key(scope, table, db_key);
};
...
void check_ordered_keys(const std::vector<T>& ordered_keys, KeyFunc key_func, bool skip_trailing_prim_key = false) {
   ...
   for (const auto& key: ordered_keys) {
      ...
      auto full_key = dcd::chain::backing_store::db_key_value_format::create_full_key(comp_key, contract);
      full_keys.push_back(full_key);
   }
...
void verify_secondary_key_pair_order(const dcd::chain::backing_store::db_key_value_format::secondary_key_pair& incoming_key_pair, const key_type kt) {
   using slice_array = std::array<rocksdb::Slice, 4>;
   const slice_array incoming {
         dcd::chain::backing_store::db_key_value_format::prefix_slice(incoming_key_pair.primary_to_secondary_key),
         dcd::chain::backing_store::db_key_value_format::prefix_type_slice(incoming_key_pair.primary_to_secondary_key),
         dcd::chain::backing_store::db_key_value_format::prefix_primary_to_secondary_slice(incoming_key_pair.primary_to_secondary_key, false),
         dcd::chain::backing_store::db_key_value_format::prefix_primary_to_secondary_slice(incoming_key_pair.primary_to_secondary_key, true)
   };
   ...
   BOOST_REQUIRE_NO_THROW(dcd::chain::backing_store::db_key_value_format::get_prefix_key(incoming_key_pair.secondary_key, scope, table));
   ...
   BOOST_REQUIRE(dcd::chain::backing_store::db_key_value_format::get_trailing_primary_key(complete_key, all_but_primary_key, primary_key));
   ...
   auto desc = [&kt, &current_kt](int comp) -> std::string {
      std::string msg = "incoming type: " + dcd::chain::backing_store::db_key_value_format::detail::to_string(kt);
      msg += ", comparing to type: " + dcd::chain::backing_store::db_key_value_format::detail::to_string(current_kt);
   ...
   auto split_keys = [](const b1::chain_kv::bytes& composite) -> slice_array {
      return {
         dcd::chain::backing_store::db_key_value_format::prefix_slice(composite),
         dcd::chain::backing_store::db_key_value_format::prefix_type_slice(composite),
         dcd::chain::backing_store::db_key_value_format::prefix_primary_to_secondary_slice(composite, false),
         dcd::chain::backing_store::db_key_value_format::prefix_primary_to_secondary_slice(composite, true)
      };
   };
   ...
   auto sec_key_pair = dcd::chain::backing_store::db_key_value_format::create_secondary_key_pair(scope, table, helper<uint64_t>().init, primary_key);
   auto values = split_keys(sec_key_pair.primary_to_secondary_key);
   compare_keys(values);
   auto prim_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_primary_to_secondary_key(scope, table, primary_key2, helper<uint64_t>().init);
   ...
   sec_key_pair = dcd::chain::backing_store::db_key_value_format::create_secondary_key_pair(scope, table, helper<dcd::chain::uint128_t>().init, primary_key);
   values = split_keys(sec_key_pair.primary_to_secondary_key);
   compare_keys(values);
   prim_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_primary_to_secondary_key(scope, table, primary_key2, helper<dcd::chain::uint128_t>().init);
   ...
   sec_key_pair = dcd::chain::backing_store::db_key_value_format::create_secondary_key_pair(scope, table, helper<dcd::chain::key256_t>().init, primary_key);
   values = split_keys(sec_key_pair.primary_to_secondary_key);
   compare_keys(values);
   prim_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_primary_to_secondary_key(scope, table, primary_key2, helper<dcd::chain::key256_t>().init);
   values2 = split_keys(prim_to_sec_key);
   // ensure that the primary_to_secondary prefix for a specific type matches
   BOOST_CHECK_EQUAL(0, compare_composite_keys(values[2], values2[2]).first);
   // ensure that their ordered by primary_key
   ...
   sec_key_pair = dcd::chain::backing_store::db_key_value_format::create_secondary_key_pair(scope, table, helper<float64_t>().init, primary_key);
   values = split_keys(sec_key_pair.primary_to_secondary_key);
   compare_keys(values);
   prim_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_primary_to_secondary_key(scope, table, primary_key2, helper<float64_t>().init);
   values2 = split_keys(prim_to_sec_key);
   // ensure that the primary_to_secondary prefix for a specific type matches
   BOOST_CHECK_EQUAL(0, compare_composite_keys(values[2], values2[2]).first);
   // ensure that their ordered by primary_key
   ...
   sec_key_pair = dcd::chain::backing_store::db_key_value_format::create_secondary_key_pair(scope, table, helper<float128_t>().init, primary_key);
   values = split_keys(sec_key_pair.primary_to_secondary_key);
   compare_keys(values);
   prim_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_primary_to_secondary_key(scope, table, primary_key2, helper<float128_t>().init);
   values2 = split_keys(prim_to_sec_key);
   // ensure that the primary_to_secondary prefix for a specific type matches
   BOOST_CHECK_EQUAL(0, compare_composite_keys(values[2], values2[2]).first);
   // ensure that their ordered by primary_key
...
void verify_primary_to_sec_key(const dcd::chain::backing_store::db_key_value_format::secondary_key_pair& key_pair, name scope, name table, uint64_t primary_key, Key sec_key) {
   ...
   const key_type actual_kt = dcd::chain::backing_store::db_key_value_format::extract_key_type(key_pair.primary_to_secondary_key);
   BOOST_REQUIRE(key_type::primary_to_sec == actual_kt);
   const key_type actual_sec_kt = dcd::chain::backing_store::db_key_value_format::extract_primary_to_sec_key_type(key_pair.primary_to_secondary_key);
   const key_type expected_sec_kt = dcd::chain::backing_store::db_key_value_format::derive_secondary_key_type<Key>();
   BOOST_REQUIRE(expected_sec_kt == actual_sec_kt);

   b1::chain_kv::bytes::const_iterator composite_loc;
   key_type kt = key_type::table;
   std::tie(extracted_scope, extracted_table, composite_loc, kt) = dcd::chain::backing_store::db_key_value_format::get_prefix_thru_key_type(key_pair.primary_to_secondary_key);
   ...
   auto result = dcd::chain::backing_store::db_key_value_format::get_primary_to_secondary_keys(
         key_pair.primary_to_secondary_key, extracted_scope, extracted_table, extracted_primary_key, extracted_key);
   BOOST_REQUIRE(dcd::chain::backing_store::db_key_value_format::prim_to_sec_type_result::valid_type == result);
   BOOST_CHECK_EQUAL(scope.to_string(), extracted_scope.to_string());
   BOOST_CHECK_EQUAL(table.to_string(), extracted_table.to_string());
   ...
   result = dcd::chain::backing_store::db_key_value_format::get_primary_to_secondary_keys(
         key_pair.primary_to_secondary_key, extracted_scope, extracted_table, primary_key, alt_key);
   BOOST_REQUIRE(dcd::chain::backing_store::db_key_value_format::prim_to_sec_type_result::wrong_sec_type == result);
   BOOST_CHECK_EQUAL(scope.to_string(), extracted_scope.to_string());
   BOOST_CHECK_EQUAL(table.to_string(), extracted_table.to_string());
   ...
   BOOST_REQUIRE(dcd::chain::backing_store::db_key_value_format::prim_to_sec_type_result::invalid_type == result);
   BOOST_CHECK_EQUAL(scope.to_string(), extracted_scope.to_string());
   BOOST_CHECK_EQUAL(table.to_string(), extracted_table.to_string());
   BOOST_CHECK_EQUAL(default_primary_key, extracted_primary_key);
...
void verify_secondary_key_pair(const b1::chain_kv::bytes& composite_key) {
   ...
   BOOST_REQUIRE_NO_THROW(dcd::chain::backing_store::db_key_value_format::get_secondary_key(composite_key, scope, table, sec_key, primary_key));
   Key sec_key2= helper<Key>{}.init;
   uint64_t primary_key2 = 0;
   const auto type_prefix = dcd::chain::backing_store::db_key_value_format::prefix_type_slice(composite_key);
   BOOST_REQUIRE_NO_THROW(dcd::chain::backing_store::db_key_value_format::get_trailing_sec_prim_keys(composite_key, type_prefix, sec_key2, primary_key2));
   BOOST_CHECK(sec_key == sec_key2);
   BOOST_CHECK_EQUAL(primary_key, primary_key2);

   const auto sec_key_pair = dcd::chain::backing_store::db_key_value_format::create_secondary_key_pair(scope, table, sec_key, primary_key);
   ...
   const auto primary_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_primary_to_secondary_key(scope, table, primary_key, sec_key);
   ...
   const key_type actual_kt = dcd::chain::backing_store::db_key_value_format::extract_key_type(sec_key_pair.secondary_key);
   BOOST_CHECK(dcd::chain::backing_store::db_key_value_format::derive_secondary_key_type<Key>() == actual_kt);
   ...
   Key extracted_key2;
   auto primary_to_sec_prefix = dcd::chain::backing_store::db_key_value_format::prefix_type_slice(primary_to_sec_key);
   auto result = dcd::chain::backing_store::db_key_value_format::get_trailing_prim_sec_keys(
   ...
   Key extracted_key3;
   primary_to_sec_prefix = dcd::chain::backing_store::db_key_value_format::prefix_primary_to_secondary_slice(primary_to_sec_key, false);
   result = dcd::chain::backing_store::db_key_value_format::get_trailing_prim_sec_keys(
   ...
   Key extracted_key4;
   primary_to_sec_prefix = dcd::chain::backing_store::db_key_value_format::prefix_primary_to_secondary_slice(primary_to_sec_key, true);
   result = dcd::chain::backing_store::db_key_value_format::get_trailing_secondary_key(
         primary_to_sec_key, primary_to_sec_prefix, extracted_key4);
   BOOST_REQUIRE(result);
   BOOST_CHECK(sec_key == extracted_key4);

   using end_of_prefix = dcd::chain::backing_store::db_key_value_format::end_of_prefix;
   uint64_t extracted_primary_key5 = default_primary_key;
   Key extracted_key5;
   name contract = "mycontract"_n;
   auto full_primary_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_full_key(primary_to_sec_key, contract);
   auto prefix_primary_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_full_key_prefix(full_primary_to_sec_key, end_of_prefix::at_type);
   result = dcd::chain::backing_store::db_key_value_format::get_trailing_prim_sec_keys(
   ...
   Key extracted_key6;
   auto type_prefix_primary_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_full_key_prefix(full_primary_to_sec_key, end_of_prefix::at_prim_to_sec_type);
   result = dcd::chain::backing_store::db_key_value_format::get_trailing_prim_sec_keys(
   ...
   Key extracted_key7;
   type_prefix_primary_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_full_key_prefix(full_primary_to_sec_key, end_of_prefix::at_prim_to_sec_primary_key);
   result = dcd::chain::backing_store::db_key_value_format::get_trailing_secondary_key(
         full_primary_to_sec_key, type_prefix_primary_to_sec_key, extracted_key7);
   BOOST_REQUIRE(result);
   BOOST_REQUIRE(sec_key == extracted_key7);

   auto full_primary_from_prim_to_sec_key = dcd::chain::backing_store::db_key_value_format::create_secondary_key_from_primary_to_sec_key<Key>(full_primary_to_sec_key);
   auto legacy_primary_from_prim_to_sec_key = dcd::chain::backing_store::db_key_value_format::extract_legacy_key(full_primary_from_prim_to_sec_key);
   ...
   const key_type kt = dcd::chain::backing_store::db_key_value_format::detail::determine_sec_type<Key>::kt;
...
BOOST_AUTO_TEST_CASE(ui64_keys_conversions_test) {
   ...
   const auto composite_key = dcd::chain::backing_store::db_key_value_format::create_primary_key(scope, table, key);
   ...
   BOOST_REQUIRE_NO_THROW(dcd::chain::backing_store::db_key_value_format::get_primary_key(composite_key, decomposed_scope, decomposed_table, decomposed_key));
   ...
   const auto composite_key2 = dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope2, table2, key2, primary_key2);
   ...
   BOOST_CHECK(dcd::chain::backing_store::db_key_value_format::get_secondary_key(composite_key2, decomposed_scope, decomposed_table, decomposed_key, decomposed_primary_key));
   ...
   check_ordered_keys(keys2, &dcd::chain::backing_store::db_key_value_format::create_secondary_key<uint64_t>);
}

BOOST_AUTO_TEST_CASE(ui128_key_conversions_test) {
   const name scope = "myscope"_n;
   const name table = "thisscope"_n;
   const dcd::chain::uint128_t key = create_uint128(0xdead12345678beef, 0x0123456789abcdef);
   const uint64_t primary_key = 0x0123456789abcdef;

   const auto composite_key = dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, key, primary_key);
   BOOST_REQUIRE_EQUAL(legacy_overhead_size + sizeof(key) + sizeof(primary_key), composite_key.size());

   name decomposed_scope;
   name decomposed_table;
   dcd::chain::uint128_t decomposed_key = 0x0;
   uint64_t decomposed_primary_key = 0x0;
   BOOST_REQUIRE_NO_THROW(dcd::chain::backing_store::db_key_value_format::get_secondary_key(composite_key, decomposed_scope, decomposed_table, decomposed_key, decomposed_primary_key));
   BOOST_CHECK_EQUAL(scope.to_string(), decomposed_scope.to_string());
   BOOST_CHECK_EQUAL(table.to_string(), decomposed_table.to_string());
   ...
   verify_secondary_key_pair<dcd::chain::uint128_t>(composite_key);

   std::vector<dcd::chain::uint128_t> keys;
   ...
   check_ordered_keys(keys, &dcd::chain::backing_store::db_key_value_format::create_secondary_key<dcd::chain::uint128_t>);
}

BOOST_AUTO_TEST_CASE(ui256_key_conversions_test) {
   const name scope = "myscope"_n;
   const name table = "thisscope"_n;
   const dcd::chain::key256_t key = create_key256(create_uint128(0xdead12345678beef, 0x0123456789abcdef),
   ...
   name decomposed_scope;
   name decomposed_table;
   dcd::chain::key256_t decomposed_key = create_key256(create_uint128(0x0, 0x0), create_uint128(0x0, 0x0));
   ...
   verify_secondary_key_pair<dcd::chain::key256_t>(composite_key);

   std::vector<dcd::chain::key256_t> keys;
   ...
   check_ordered_keys(keys, &dcd::chain::backing_store::db_key_value_format::create_secondary_key<dcd::chain::key256_t>);
}
...
BOOST_AUTO_TEST_CASE(float64_key_conversions_test) {
   ...
   keys.push_back(to_softfloat64(99999.9));
   keys.push_back(to_softfloat64(100000.0));
   check_ordered_keys(keys, &dcd::chain::backing_store::db_key_value_format::create_secondary_key<float64_t>);
}
...
BOOST_AUTO_TEST_CASE(float128_key_conversions_test) {
   ...
   keys.push_back(to_softfloat128(100000.0));
   keys.push_back(to_softfloat128(100000.001));
   check_ordered_keys(keys, &dcd::chain::backing_store::db_key_value_format::create_secondary_key<float128_t>);
}
...
BOOST_AUTO_TEST_CASE(table_key_conversions_test) {
   const name scope = "myscope"_n;
   const name table = "thisscope"_n;
   const auto composite_key = dcd::chain::backing_store::db_key_value_format::create_table_key(scope, table);
   BOOST_REQUIRE_EQUAL(legacy_overhead_size, composite_key.size());

   name decomposed_scope;
   name decomposed_table;
   BOOST_REQUIRE_NO_THROW(dcd::chain::backing_store::db_key_value_format::get_table_key(composite_key, decomposed_scope, decomposed_table));
   BOOST_CHECK_EQUAL(scope.to_string(), decomposed_scope.to_string());
   BOOST_CHECK_EQUAL(table.to_string(), decomposed_table.to_string());
...
BOOST_AUTO_TEST_CASE(prefix_key_conversions_test) {
   const name scope = "myscope"_n;
   const name table = "thisscope"_n;
   const auto composite_key = dcd::chain::backing_store::db_key_value_format::create_prefix_key(scope, table);
   BOOST_REQUIRE_EQUAL(legacy_overhead_size_without_type, composite_key.size());  //

   name decomposed_scope;
   name decomposed_table;
   BOOST_REQUIRE_NO_THROW(dcd::chain::backing_store::db_key_value_format::get_prefix_key(composite_key, decomposed_scope, decomposed_table));
   BOOST_CHECK_EQUAL(scope.to_string(), decomposed_scope.to_string());
   BOOST_CHECK_EQUAL(table.to_string(), decomposed_table.to_string());
}
...
BOOST_AUTO_TEST_CASE(compare_key_type_order_test) {
   ...
   const dcd::chain::uint128_t max_i128 = create_uint128(max, max);
   const dcd::chain::key256_t max_i256 = create_key256(create_uint128(max, max), create_uint128(max, max));
   ...
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_prefix_key(scope, table));
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_primary_key(scope, table, 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_primary_key(scope, table, max));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, uint64_t(0x0), 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, max, 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, create_uint128(0x0, 0x0), 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, max_i128, 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, create_key256(create_uint128(0x0, 0x0), create_uint128(0x0, 0x0)), 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, max_i256, 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, to_softfloat64(0.0), 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, max_f64, 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, to_softfloat128(0.0), 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_secondary_key(scope, table, max_f128, 0x0));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
   composite_keys.push_back(dcd::chain::backing_store::db_key_value_format::create_table_key(scope, table));
   BOOST_CHECK_EQUAL(-1, compare_composite_keys(composite_keys, next++).first);
}
...
void verify_primary_exception(const b1::chain_kv::bytes& composite_key, const std::string& starts_with) {
   ...
   BOOST_CHECK_EXCEPTION(
         dcd::chain::backing_store::db_key_value_format::get_primary_key(composite_key, decomposed_scope, decomposed_table, decomposed_key),
         Exception,
         dcd::testing::fc_exception_message_starts_with(starts_with)
   );
}
...
void verify_secondary_exception(const b1::chain_kv::bytes& composite_key, const std::string& starts_with) {
   ...
   uint64_t decomposed_primary_key = 0;
   BOOST_CHECK_EXCEPTION(
         dcd::chain::backing_store::db_key_value_format::get_secondary_key(composite_key, decomposed_scope, decomposed_table, decomposed_key, decomposed_primary_key),
         Exception,
         dcd::testing::fc_exception_message_starts_with(starts_with)
   );
}
...
BOOST_AUTO_TEST_CASE(verify_no_type_in_key_test) {
   ...
   verify_primary_exception<dcd::chain::bad_composite_key_exception>(composite_key, starts_with);
   verify_secondary_exception<uint64_t, dcd::chain::bad_composite_key_exception>(composite_key, starts_with);
   verify_secondary_exception<dcd::chain::uint128_t, dcd::chain::bad_composite_key_exception>(composite_key, starts_with);
   verify_secondary_exception<dcd::chain::key256_t, dcd::chain::bad_composite_key_exception>(composite_key, starts_with);
   verify_secondary_exception<float64_t, dcd::chain::bad_composite_key_exception>(composite_key, starts_with);
   verify_secondary_exception<float128_t, dcd::chain::bad_composite_key_exception>(composite_key, starts_with);
}
...
BOOST_AUTO_TEST_CASE(table_cache_test) {
   ...
   BOOST_CHECK_THROW(i64_cache.get_end_iterator_by_table(t5), dcd::chain::table_not_in_cache);
   BOOST_CHECK_THROW(i64_cache.find_table_by_end_iterator(i64_cache.invalid_iterator()), dcd::chain::invalid_table_iterator);
...
BOOST_AUTO_TEST_CASE(invalid_itr_cache_test) {
   db_key_value_iter_store<dcd::chain::uint128_t> i128_cache;
   BOOST_CHECK_EXCEPTION(
      i128_cache.get( i128_cache.invalid_iterator() ), dcd::chain::invalid_table_iterator,
      dcd::testing::fc_exception_message_is( "invalid iterator" )
   );
   BOOST_CHECK_EXCEPTION(
      i128_cache.get( i128_cache.invalid_iterator() - 1 ), dcd::chain::table_operation_not_permitted,
      dcd::testing::fc_exception_message_is( "dereference of end iterator" )
   );
   BOOST_CHECK_EXCEPTION(
      i128_cache.get( 0 ), dcd::chain::invalid_table_iterator,
      dcd::testing::fc_exception_message_is( "iterator out of range" )
   );

   BOOST_CHECK_EXCEPTION(
      i128_cache.remove( i128_cache.invalid_iterator() ), dcd::chain::invalid_table_iterator,
      dcd::testing::fc_exception_message_is( "invalid iterator" )
   );
   BOOST_CHECK_EXCEPTION(
      i128_cache.remove( i128_cache.invalid_iterator() - 1 ), dcd::chain::table_operation_not_permitted,
      dcd::testing::fc_exception_message_is( "cannot call remove on end iterators" )
   );
   BOOST_CHECK_EXCEPTION(
      i128_cache.remove( 0 ), dcd::chain::invalid_table_iterator,
      dcd::testing::fc_exception_message_is( "iterator out of range" )
   );

   dcd::chain::uint128_t key = 0;
   dcd::chain::account_name payer;
   BOOST_CHECK_EXCEPTION(
      i128_cache.swap( i128_cache.invalid_iterator(), key, payer ), dcd::chain::invalid_table_iterator,
      dcd::testing::fc_exception_message_is( "invalid iterator" )
   );
   BOOST_CHECK_EXCEPTION(
      i128_cache.swap( i128_cache.invalid_iterator() - 1, key, payer ), dcd::chain::table_operation_not_permitted,
      dcd::testing::fc_exception_message_is( "cannot call swap on end iterators" )
   );
   BOOST_CHECK_EXCEPTION(
      i128_cache.swap( 0, key, payer ), dcd::chain::invalid_table_iterator,
      dcd::testing::fc_exception_message_is( "iterator out of range" )
   );

   using i128_type = secondary_key<dcd::chain::uint128_t>;
   const i128_type i128_1 = { .table_ei = i128_cache.invalid_iterator(), .secondary = 0x0, .primary = 0x0, .payer = name{} };
   BOOST_CHECK_EXCEPTION(
      i128_cache.add( i128_1 ), dcd::chain::invalid_table_iterator,
      dcd::testing::fc_exception_message_is( "not an end iterator" )
   );
   const i128_type i128_2 = { .table_ei = i128_cache.invalid_iterator() - 1, .secondary = 0x0, .primary = 0x0, .payer = name{} };
   BOOST_CHECK_EXCEPTION(
      i128_cache.add( i128_2 ), dcd::chain::invalid_table_iterator,
      dcd::testing::fc_exception_message_is( "an invariant was broken, table should be in cache" )
   );
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(itr_cache_test, KEY_SUITE, key_suite) {
   ...
   BOOST_CHECK_EXCEPTION(
      key_cache.get(key_1_itr), dcd::chain::table_operation_not_permitted,
      dcd::testing::fc_exception_message_is( "dereference of deleted object" )
   );
