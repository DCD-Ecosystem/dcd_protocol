...
class dcd_token_tester : public tester {
public:

   dcd_token_tester() {
      produce_blocks( 2 );

      create_accounts( { "alice"_n, "bob"_n, "carol"_n, "dcd.token"_n } );
      produce_blocks( 2 );
   ...
   {
      auto symb = dcd::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( "dcd.token"_n, name(symbol_code), "stat"_n, name(symbol_code) );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = dcd::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( "dcd.token"_n, acc, "accounts"_n, name(symbol_code) );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( "dcd.token"_n, "create"_n, mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(dcd_token_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, dcd_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, dcd_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, dcd_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, dcd_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, dcd_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, dcd_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, dcd_token_tester ) try {
