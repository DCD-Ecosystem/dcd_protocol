...
proxy::proxy( dcd::name self, dcd::name first_receiver, dcd::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      dcd::token::transfer_action a( "dcd.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, dcd::ignore<std::vector<char>> ) {
