...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance("dcd.token"_n, symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action("dcd.token"_n, "issue"_n, mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::dcd_token_abi().data()).as<abi_def>(), abi_serializer::create_yield_function( abi_serializer_max_time ))
      {
         create_account( "dcd.token"_n);
         set_code( "dcd.token"_n, contracts::dcd_token_wasm() );

         auto result = push_action("dcd.token"_n, "create"_n, mutable_variant_object()
                 ("issuer",       dcd_token)
         ...
         result = push_action("dcd.token"_n, "issue"_n, mutable_variant_object()
                 ("to",       dcd_token)
      ...
      static const name dcd_token;
};

const name currency_tester::dcd_token = "dcd.token"_n;
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance("dcd.token"_n, expected.get_symbol(), "dcd.token"_n);
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action("dcd.token"_n, "transfer"_n, mutable_variant_object()
         ("from", dcd_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action("dcd.token"_n, "transfer"_n, mutable_variant_object()
                                    ("from", dcd_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_overspend, currency_tester ) try {
   ...
   // Overspend!
   {
      ...
      BOOST_CHECK_EXCEPTION( push_action("alice"_n, "transfer"_n, data),
                             dcd_assert_message_exception, dcd_assert_message_is("overdrawn balance") );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      symbol sys(4, "DCD");
      BOOST_REQUIRE_EQUAL(SY(4,DCD), sys.value());
      BOOST_REQUIRE_EQUAL("4,DCD", sys.to_string());
      BOOST_REQUIRE_EQUAL("DCD", sys.name());
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(dcd_token, "alice"_n, "100.0000 CUR");
   ...
   {
      BOOST_REQUIRE_THROW(transfer("alice"_n, "carl"_n, "20.50 USD"), dcd_assert_message_exception);
   }
