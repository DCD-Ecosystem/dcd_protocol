...
#define REQUIRE_MATCHING_OBJECT(left, right) { auto a = fc::variant( left ); auto b = fc::variant( right ); BOOST_REQUIRE_EQUAL( true, a.is_object() ); \
   BOOST_REQUIRE_EQUAL( true, b.is_object() ); \
   auto filtered = ::dcd::testing::filter_fields( a.get_object(), b.get_object() ); \
...
dcd::chain::asset core_from_string(const std::string& s);
...
namespace dcd { namespace testing {
   ...
   class base_tester {
      public:
         ...
         static std::pair<controller::config, genesis_state> default_config(
            ...
            cfg.dcdvmoc_config.cache_size = 1024*1024*8;

            for(int i = 0; i < boost::unit_test::framework::master_test_suite().argc; ++i) {
               if(boost::unit_test::framework::master_test_suite().argv[i] == std::string("--dcd-vm"))
                  cfg.wasm_runtime = chain::wasm_interface::vm_type::dcd_vm;
               else if(boost::unit_test::framework::master_test_suite().argv[i] == std::string("--dcd-vm-jit"))
                  cfg.wasm_runtime = chain::wasm_interface::vm_type::dcd_vm_jit;
               else if(boost::unit_test::framework::master_test_suite().argv[i] == std::string("--dcd-vm-oc"))
                  cfg.wasm_runtime = chain::wasm_interface::vm_type::dcd_vm_oc;
            }
  ...
  /**
   * Utility predicate to check whether an dcd_assert message is equivalent to a given string
   */
  struct dcd_assert_message_is {
     dcd_assert_message_is( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }

     bool operator()( const dcd_assert_message_exception& ex );

     string expected;
  };

  /**
   * Utility predicate to check whether an dcd_assert message starts with a given string
   */
  struct dcd_assert_message_starts_with {
     dcd_assert_message_starts_with( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }
  ...
  /**
   * Utility predicate to check whether an dcd_assert_code error code is equivalent to a given number
   */
  struct dcd_assert_code_is {
     dcd_assert_code_is( uint64_t error_code )
           : expected( "assertion failure with error code: " ) {
        expected.append( std::to_string(error_code) );
     }

     bool operator()( const dcd_assert_code_exception& ex );
