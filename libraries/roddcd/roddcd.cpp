...
#include <b1/roddcd/roddcd_tables.hpp>
...
roddcd_db_snapshot::roddcd_db_snapshot(std::shared_ptr<roddcd_db_partition> partition, bool persistent)
...
void roddcd_db_snapshot::refresh() {
   if (undo_stack)
...
void roddcd_db_snapshot::write_fill_status() {
   if (!undo_stack)
...
void roddcd_db_snapshot::end_write(bool write_fill) {
   if (!undo_stack)
      throw std::runtime_error("Can only write to persistent snapshots");
   if (write_fill)
      write_fill_status();
...
void roddcd_db_snapshot::start_block(const get_blocks_result_base& result) {
   if (!undo_stack)
      throw std::runtime_error("Can only write to persistent snapshots");
...
void roddcd_db_snapshot::end_block(const get_blocks_result_base& result, bool force_write) {
   if (!undo_stack)
      throw std::runtime_error("Can only write to persistent snapshots");
   if (!result.this_block)
      throw std::runtime_error("get_blocks_result this_block is empty");
...
void roddcd_db_snapshot::check_write(const ship_protocol::get_blocks_result_base& result) {
   if (!undo_stack)
      throw std::runtime_error("Can only write to persistent snapshots");
   if (!result.this_block)
      throw std::runtime_error("get_blocks_result this_block is empty");
...
void roddcd_db_snapshot::write_block_info(uint32_t block_num, const dcd::checksum256& id,
...
void roddcd_db_snapshot::write_block_info(const ship_protocol::get_blocks_result_v0& result) {
   check_write(result);
   if (!result.block)
      return;
   ...
   auto blk_span = fc_create_span( blk_trace, "roddcd-received" );
...
void roddcd_db_snapshot::write_block_info(const ship_protocol::get_blocks_result_v1& result) {
   check_write(result);
   if (!result.block)
      return;
...
void roddcd_db_snapshot::write_deltas(uint32_t block_num, dcd::opaque<std::vector<ship_protocol::table_delta>> deltas, std::function<bool()> shutdown) {
   db_view_state view_state{ state_account, *db, *write_session, partition->contract_kv_prefix };
...
void roddcd_db_snapshot::write_deltas(const ship_protocol::get_blocks_result_v0& result,
...
void roddcd_db_snapshot::write_deltas(const ship_protocol::get_blocks_result_v1& result,
                                      std::function<bool()> shutdown) {
   check_write(result);
...
roddcd_filter::roddcd_filter(dcd::name name, const std::string& wasm_filename) : name{ name } {
...
void roddcd_filter::process(roddcd_db_snapshot& snapshot, const ship_protocol::get_blocks_result_base& result,
...
roddcd_query_handler::roddcd_query_handler(std::shared_ptr<roddcd_db_partition>         partition,
