...
#define DCD_CDT_GET_RETURN_T(value_class, index_name) std::decay_t<decltype(std::invoke(&value_class::index_name, std::declval<const value_class*>()))>
...
#define KV_NAMED_INDEX(index_name, member_name)                                                                        \
   index<DCD_CDT_GET_RETURN_T(value_type, member_name)> member_name{dcd::name{index_name}, &value_type::member_name};
...
namespace detail {
   ...
   static void deserialize(V& value, void* buffer, size_t size) {
      ...
      ds >> idx;
      dcd::check(idx==unsigned_int(0), "there was an error deserializing this value.");
      ds >> value;
   }
...
inline key_type make_prefix(dcd::name table_name, dcd::name index_name, uint8_t status = 1) {
...
#if DCD_CDT_DOXYGEN
/**
 * @brief A function for converting types to the appropriate binary representation for the DCD Key Value database.
...
static constexpr dcd::name kv_ram = "dcd.kvram"_n;
static constexpr dcd::name kv_disk = "dcd.kvdisk"_n;
 ...
 * @ingroup contracts
 *
 * @brief Defines an DCD Key Value Table
 * @details DCD Key Value API provides a C++ interface to the DCD Key Value database.
...
class kv_table {
   ...
   public:
      ...
      T value() const {
         ...
         dcd::check(itr_stat != status::iterator_end, "Cannot read end iterator");
         ...
         dcd::check(static_cast<status>(stat) == status::iterator_ok, "Error reading value");
         ...
         if (!is_primary) {
            ...
            dcd::check(success, "failure getting primary key in `value()`");
      ...
      key_type key() const {
         ...
         dcd::check(static_cast<status>(stat) == status::iterator_ok, "Error getting key");

         return {(char*)buffer, actual_value_size};
      }

      iterator& operator++() {
         dcd::check(itr_stat != status::iterator_end, "cannot increment end iterator");
      ...
      iterator& operator--() {
         ...
         dcd::check(itr_stat != status::iterator_end, "decremented past the beginning");
         return *this;
      }
   ...
   protected:
      ...
      kv_index(dcd::name index_name, KF&& kf) : index_name{index_name} {
...
public:
    ...
    * @ingroup keyvalue
    *
    * @brief Defines an index on an DCD Key Value Table
   ...
   public:
      ...
      template <typename KF>
      index(dcd::name name, KF&& kf) : kv_index{name, kf} {
      ...
      T operator[](const key_type& key) const {
         auto opt = get(key);
         dcd::check(opt, __FILE__ ":" + std::to_string(__LINE__) + " Key not found in `[]`");
         return *opt;
      }
      ...
      std::optional<T> get(const key_type& k ) const {
         ...
         if (!is_primary) {
            ...
            dcd::check(success, "failure getting primary key");
   ...
   void put(const T& value) {
      ...
      for (const auto& idx : secondary_indices) {
         ...
         if (!primary_key_found) {
            dcd::check(!sec_found, "Attempted to store an existing secondary index.");
            ...
            if (sec_found) {
               ...
               dcd::check(copy_size == tbl_key.size() && res == 0, "Attempted to update an existing secondary index.");
...
protected:
   ...
   void init(dcd::name contract, dcd::name table, dcd::name db, PrimaryIndex& prim_index, SecondaryIndices&... indices) {
...
template <typename T, dcd::name::raw SingletonName, dcd::name::raw DbName = "dcd.kvram"_n>
class kv_singleton {
   kv_environment environment;
...
public:
   ...
   explicit kv_singleton(dcd::name contract_name, bool write_on_destruct) : contract_name{contract_name}, write_on_destruct{write_on_destruct} {
      key = make_prefix();
   }

   kv_singleton(dcd::name contract_name, kv_environment environment, bool write_on_destruct) : environment{environment}, contract_name{contract_name}, write_on_destruct{write_on_destruct} {}
   ...
   const T& get() {
      ...
      if (!ste.is_cached) {
         ...
         dcd::check(success, "tried to get a singleton that does not exist");
