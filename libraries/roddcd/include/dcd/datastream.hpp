   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        dcd::check( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        dcd::check( _end - _pos >= (int32_t)s, "write" );
...
namespace _datastream_detail {
   ...
   namespace operator_detection {
      // These overloads will be ambiguous with the operator in dcd, unless
