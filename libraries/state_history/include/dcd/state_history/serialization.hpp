...
struct trace_receipt_context {
   uint8_t  failed_status = dcd::chain::transaction_receipt_header::hard_fail;
...
ST& history_serialize_container(ST& ds, const chainbase::database& db, const std::vector<std::shared_ptr<T>>& v) {
   fc::raw::pack(ds, unsigned_int(v.size()));
   for (auto& x : v) {
      DCD_ASSERT(!!x, dcd::chain::plugin_exception, "null inside container");
      ds << make_history_serial_wrapper(db, *x);
   }
   return ds;
}
...
template <typename ST, typename T>
ST& operator<<(ST& ds, const dcd::state_history::big_vector_wrapper<T>& obj) {
...
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::account_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   fc::raw::pack(ds, as_type<dcd::chain::block_timestamp_type>(obj.obj.creation_date));
   fc::raw::pack(ds, as_type<dcd::chain::shared_string>(obj.obj.abi));
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::account_metadata_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   fc::raw::pack(ds, as_type<bool>(obj.obj.is_privileged()));
   fc::raw::pack(ds, as_type<fc::time_point>(obj.obj.last_code_update));
   bool has_code = obj.obj.code_hash != dcd::chain::digest_type();
   fc::raw::pack(ds, has_code);
   if (has_code) {
      fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_type));
      fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_version));
      fc::raw::pack(ds, as_type<dcd::chain::digest_type>(obj.obj.code_hash));
   }
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::code_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_type));
   fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_version));
   fc::raw::pack(ds, as_type<dcd::chain::digest_type>(obj.obj.code_hash));
   fc::raw::pack(ds, as_type<dcd::chain::shared_string>(obj.obj.code));
   return ds;
}
...
template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::table_id_object>& obj) {
   return serialize_table_id_object(ds, obj);
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::backing_store::table_id_object_view>& obj) {
   return serialize_table_id_object(ds, obj);
}

template <typename ST>
ST& operator<<(ST&                                                                                           ds,
               const history_context_wrapper<dcd::chain::table_id_object, dcd::chain::key_value_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   ...
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.payer.to_uint64_t()));
   fc::raw::pack(ds, as_type<dcd::chain::shared_string>(obj.obj.value));
   return ds;
}

template <typename ST>
ST& operator<<(ST&                                                                                           ds,
               const history_context_wrapper<dcd::chain::backing_store::table_id_object_view, dcd::chain::backing_store::primary_index_view>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.context.code.to_uint64_t()));
   fc::raw::pack(ds, as_type<uint64_t>(obj.context.scope.to_uint64_t()));
   fc::raw::pack(ds, as_type<uint64_t>(obj.context.table.to_uint64_t()));
...
template <typename ST>
void serialize_secondary_index_data(ST& ds, const dcd::chain::key256_t& obj) {
...
template <typename ST>
ST& operator<<(ST&                                                                                         ds,
               const history_context_wrapper<dcd::chain::table_id_object, dcd::chain::index64_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(ST&                                                                                         ds,
               const history_context_wrapper<dcd::chain::backing_store::table_id_object_view, dcd::chain::backing_store::secondary_index_view<uint64_t>>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(ST&                                                                                          ds,
               const history_context_wrapper<dcd::chain::table_id_object, dcd::chain::index128_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(ST&                                                                                         ds,
               const history_context_wrapper<dcd::chain::backing_store::table_id_object_view, dcd::chain::backing_store::secondary_index_view<dcd::chain::uint128_t>>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(ST&                                                                                          ds,
               const history_context_wrapper<dcd::chain::table_id_object, dcd::chain::index256_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(ST&                                                                                         ds,
               const history_context_wrapper<dcd::chain::backing_store::table_id_object_view, dcd::chain::backing_store::secondary_index_view<dcd::chain::key256_t>>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(ST&                                                                                              ds,
               const history_context_wrapper<dcd::chain::table_id_object, dcd::chain::index_double_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(ST&                                                                                         ds,
               const history_context_wrapper<dcd::chain::backing_store::table_id_object_view, dcd::chain::backing_store::secondary_index_view<float64_t>>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(
    ST& ds, const history_context_wrapper<dcd::chain::table_id_object, dcd::chain::index_long_double_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(ST&                                                                                         ds,
               const history_context_wrapper<dcd::chain::backing_store::table_id_object_view, dcd::chain::backing_store::secondary_index_view<float128_t>>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::kv_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.contract.to_uint64_t()));
   fc::raw::pack(ds, as_type<dcd::chain::shared_blob>(obj.obj.kv_key));
   fc::raw::pack(ds, as_type<dcd::chain::shared_blob>(obj.obj.kv_value));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.payer.to_uint64_t()));
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::kv_object_view>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.contract.to_uint64_t()));
...
template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::shared_block_signing_authority_v0>& obj) {
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.threshold));
   history_serialize_container(ds, obj.db,
                               as_type<dcd::chain::shared_vector<dcd::chain::shared_key_weight>>(obj.obj.keys));
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::shared_producer_authority>& obj) {
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.producer_name.to_uint64_t()));
   fc::raw::pack(ds, as_type<dcd::chain::shared_block_signing_authority>(obj.obj.authority));
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::shared_producer_authority_schedule>& obj) {
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.version));
   history_serialize_container(
       ds, obj.db, as_type<dcd::chain::shared_vector<dcd::chain::shared_producer_authority>>(obj.obj.producers));
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::chain_config>& obj) {
...
template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::global_property_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(1));
   fc::raw::pack(ds, as_type<std::optional<dcd::chain::block_num_type>>(obj.obj.proposed_schedule_block_num));
   fc::raw::pack(ds, make_history_serial_wrapper(
                         obj.db, as_type<dcd::chain::shared_producer_authority_schedule>(obj.obj.proposed_schedule)));
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<dcd::chain::chain_config>(obj.obj.configuration)));
   fc::raw::pack(ds, as_type<dcd::chain::chain_id_type>(obj.obj.chain_id));

   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::generated_transaction_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.sender.to_uint64_t()));
   fc::raw::pack(ds, as_type<__uint128_t>(obj.obj.sender_id));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.payer.to_uint64_t()));
   fc::raw::pack(ds, as_type<dcd::chain::transaction_id_type>(obj.obj.trx_id));
   fc::raw::pack(ds, as_type<dcd::chain::shared_string>(obj.obj.packed_trx));
   return ds;
}

template <typename ST>
ST& operator<<(ST&                                                                                            ds,
               const history_serial_wrapper<dcd::chain::protocol_state_object::activated_protocol_feature>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<dcd::chain::digest_type>(obj.obj.feature_digest));
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.activation_block_num));
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::protocol_state_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::shared_key_weight>& obj) {
   fc::raw::pack(ds, as_type<dcd::chain::public_key_type>(obj.obj.key));
   fc::raw::pack(ds, as_type<uint16_t>(obj.obj.weight));
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::permission_level>& obj) {
...
template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::permission_level_weight>& obj) {
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<dcd::chain::permission_level>(obj.obj.permission)));
   fc::raw::pack(ds, as_type<uint16_t>(obj.obj.weight));
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::wait_weight>& obj) {
...
template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::shared_authority>& obj) {
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.threshold));
...
template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::permission_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.owner.to_uint64_t()));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   if (obj.obj.parent._id) {
      auto&       index  = obj.db.get_index<dcd::chain::permission_index>();
      ...
      if (!parent) {
         ...
         DCD_ASSERT(it != undo.removed_values.end(), dcd::chain::plugin_exception,
   ...
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<dcd::chain::shared_authority>(obj.obj.auth)));
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::permission_link_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
//template <typename ST>
//ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::resource_limits::resource_limits_object>& obj) {
//   DCD_ASSERT(!obj.obj.pending, dcd::chain::plugin_exception,
...
//template <typename ST>
//ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::resource_limits::usage_accumulator>& obj) {
//   fc::raw::pack(ds, fc::unsigned_int(0));
...
//template <typename ST>
//ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::resource_limits::resource_usage_object>& obj) {
//   fc::raw::pack(ds, fc::unsigned_int(0));
//   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.owner.to_uint64_t()));
//   fc::raw::pack(ds, make_history_serial_wrapper(
//                         obj.db, as_type<dcd::chain::resource_limits::usage_accumulator>(obj.obj.net_usage)));
//   fc::raw::pack(ds, make_history_serial_wrapper(
//                         obj.db, as_type<dcd::chain::resource_limits::usage_accumulator>(obj.obj.cpu_usage)));
//   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.ram_usage));
//   return ds;
//}

//template <typename ST>
//ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::resource_limits::resource_limits_state_object>& obj) {
//   fc::raw::pack(ds, fc::unsigned_int(0));
//   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<dcd::chain::resource_limits::usage_accumulator>(
...
//template <typename ST>
//ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::resource_limits::ratio>& obj) {
//   fc::raw::pack(ds, fc::unsigned_int(0));
...
//template <typename ST>
//ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::resource_limits::elastic_limit_parameters>& obj) {
//   fc::raw::pack(ds, fc::unsigned_int(0));
...
//   fc::raw::pack(
//       ds, make_history_serial_wrapper(obj.db, as_type<dcd::chain::resource_limits::ratio>(obj.obj.contract_rate)));
//   fc::raw::pack(
//       ds, make_history_serial_wrapper(obj.db, as_type<dcd::chain::resource_limits::ratio>(obj.obj.expand_rate)));
//   return ds;
//}

//template <typename ST>
//ST& operator<<(ST&                                                                                         ds,
//               const history_serial_wrapper<dcd::chain::resource_limits::resource_limits_config_object>& obj) {
//   fc::raw::pack(ds, fc::unsigned_int(0));
//   fc::raw::pack(
//       ds, make_history_serial_wrapper(
//               obj.db, as_type<dcd::chain::resource_limits::elastic_limit_parameters>(obj.obj.cpu_limit_parameters)));
//   fc::raw::pack(
//       ds, make_history_serial_wrapper(
//               obj.db, as_type<dcd::chain::resource_limits::elastic_limit_parameters>(obj.obj.net_limit_parameters)));
...
template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::action>& obj) {
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.account.to_uint64_t()));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   history_serialize_container(ds, obj.db, as_type<std::vector<dcd::chain::permission_level>>(obj.obj.authorization));
   fc::raw::pack(ds, as_type<dcd::chain::bytes>(obj.obj.data));
   return ds;
}

template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::action_receipt>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.receiver.to_uint64_t()));
   fc::raw::pack(ds, as_type<dcd::chain::digest_type>(obj.obj.act_digest));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.global_sequence));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.recv_sequence));
   history_serialize_container(ds, obj.db, as_type<flat_map<dcd::chain::name, uint64_t>>(obj.obj.auth_sequence));
...
template <typename ST>
ST& operator<<(ST& ds, const history_serial_wrapper<dcd::chain::account_delta>& obj) {
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.account.to_uint64_t()));
...
inline std::optional<uint64_t> cap_error_code(const std::optional<uint64_t>& error_code) {
   ...
   const uint64_t upper_limit = static_cast<uint64_t>(dcd::chain::system_error_code::generic_system_error);
...
template <typename ST>
ST& operator<<(ST& ds, const history_context_wrapper<bool, dcd::chain::action_trace>& obj) {
   ...
   if (obj.obj.receipt) {
      fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<dcd::chain::action_receipt>(*obj.obj.receipt)));
   }
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.receiver.to_uint64_t()));
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<dcd::chain::action>(obj.obj.act)));
   ...
   history_serialize_container(ds, obj.db, as_type<flat_set<dcd::chain::account_delta>>(obj.obj.account_ram_deltas));
   history_serialize_container(ds, obj.db, as_type<flat_set<dcd::chain::account_delta>>(obj.obj.account_disk_deltas));
   ...
   fc::raw::pack(ds, as_type<dcd::chain::bytes>(obj.obj.return_value));
   return ds;
}
...
ST& operator<<(
    ST&                                                                                                      ds,
    const history_context_wrapper<trace_receipt_context, dcd::state_history::augmented_transaction_trace>& obj) {
   auto& trace      = *obj.obj.trace;
   bool  debug_mode = obj.context.debug_mode;
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<dcd::chain::transaction_id_type>(trace.id));
   if (trace.receipt) {
      if (trace.failed_dtrx_trace && trace.receipt->status.value == dcd::chain::transaction_receipt_header::soft_fail)
         fc::raw::pack(ds, uint8_t(dcd::chain::transaction_receipt_header::executed));
      else
   ...
   history_context_serialize_container(ds, obj.db, debug_mode,
                                       as_type<std::vector<dcd::chain::action_trace>>(trace.action_traces));

   fc::raw::pack(ds, bool(trace.account_ram_delta));
   if (trace.account_ram_delta) {
      fc::raw::pack(
          ds, make_history_serial_wrapper(obj.db, as_type<dcd::chain::account_delta>(*trace.account_ram_delta)));
   }
   ...
   if (trace.failed_dtrx_trace) {
      ...
      context.failed_status         = dcd::chain::transaction_receipt_header::hard_fail;
      if (trace.receipt && trace.receipt->status.value == dcd::chain::transaction_receipt_header::soft_fail)
         context.failed_status = dcd::chain::transaction_receipt_header::soft_fail;
      fc::raw::pack( //
          ds, make_history_context_wrapper(
                  obj.db, context,
                  dcd::state_history::augmented_transaction_trace{trace.failed_dtrx_trace, obj.obj.packed_trx}));
   }
   ...
   if (include_packed_trx) {
      ...
      fc::raw::pack(ds, fc::unsigned_int( fc::get_index<dcd::state_history::partial_transaction, dcd::state_history::partial_transaction_v1>() ));
      fc::raw::pack(ds, as_type<dcd::chain::time_point_sec>(trx.expiration));
      ...
      fc::raw::pack(ds, as_type<dcd::chain::extensions_type>(trx.transaction_extensions));
      fc::raw::pack(ds, as_type<std::optional<dcd::state_history::prunable_data_type>>({}));
   }
...
template <typename ST>
ST& operator<<(ST& ds, const dcd::state_history::get_blocks_result_v0& obj) {
   fc::raw::pack(ds, obj.head);
   fc::raw::pack(ds, obj.last_irreversible);
   fc::raw::pack(ds, obj.this_block);
   fc::raw::pack(ds, obj.prev_block);
   dcd::state_history::pack_big_bytes(ds, obj.block);
   dcd::state_history::pack_big_bytes(ds, obj.traces);
   dcd::state_history::pack_big_bytes(ds, obj.deltas);
   return ds;
}

template <typename ST>
void pack_for_blocks_result_v1(ST& ds, const dcd::state_history::signed_block_ptr_variant& obj) {
...
template <typename ST>
void pack_for_blocks_result_v2(ST& ds, const dcd::state_history::signed_block_ptr_variant& obj) {
   uint8_t which = obj.index();
   std::visit([&ds, which](const auto& ptr) {
      if (ptr) {
...
template <typename ST>
ST& operator<<(ST& ds, const dcd::state_history::get_blocks_result_v1& obj) {
   fc::raw::pack(ds, obj.head);
   fc::raw::pack(ds, obj.last_irreversible);
   fc::raw::pack(ds, obj.this_block);
   fc::raw::pack(ds, obj.prev_block);
...
template <typename ST>
ST& operator<<(ST& ds, const dcd::state_history::get_blocks_result_v2& obj) {
   fc::raw::pack(ds, obj.head);
   fc::raw::pack(ds, obj.last_irreversible);
   fc::raw::pack(ds, obj.this_block);
   fc::raw::pack(ds, obj.prev_block);
