...
struct ondisk_prunable_full_legacy {
   ...
   dcd::chain::digest_type   digest;
   dcd::chain::digest_type   prunable_digest() const { return digest; }
};
...
void for_each_packed_transaction(const dcd::state_history::augmented_transaction_trace& obj, const Lambda& lambda) {
   auto& trace = *obj.trace;
   if (trace.failed_dtrx_trace) {
      for_each_packed_transaction(
          dcd::state_history::augmented_transaction_trace{trace.failed_dtrx_trace, obj.packed_trx}, lambda);
   }
...
template <typename Lambda>
void for_each_packed_transaction(const std::vector<dcd::state_history::augmented_transaction_trace>& traces,
                                 const Lambda&                                                         lambda) {
...
FC_REFLECT(dcd::state_history::trace_converter::ondisk_prunable_full_legacy,
