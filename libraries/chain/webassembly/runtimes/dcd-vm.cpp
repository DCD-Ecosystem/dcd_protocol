...
// sure that dcd-vm will replicate WAVM's parsing exactly.
...
void validate(const bytes& code, const whitelisted_intrinsics_type& intrinsics) {
   ...
   try {
      dcd_vm_null_backend_t<setcode_options> bkend(code_ptr, code.size(), nullptr);
      // check import signatures
       dcd_vm_host_functions_t::resolve(bkend.get_module());
      ...
      for(std::uint32_t i = 0; i < imports.size(); ++i) {
         DCD_ASSERT(std::string_view((char*)imports[i].module_str.raw(), imports[i].module_str.size()) == "env" &&
      ...
      DCD_THROW(wasm_serialization_error, e.detail());
   }
}

void validate( const bytes& code, const wasm_config& cfg, const whitelisted_intrinsics_type& intrinsics ) {
   DCD_ASSERT(code.size() <= cfg.max_module_bytes, wasm_serialization_error, "Code too large");
   wasm_code_ptr code_ptr((uint8_t*)code.data(), code.size());
   try {
      dcd_vm_null_backend_t<wasm_config> bkend(code_ptr, code.size(), nullptr, cfg);
      // check import signatures
      dcd_vm_host_functions_t::resolve(bkend.get_module());
      // check that the imports are all currently enabled
      const auto& imports = bkend.get_module().imports;
      ...
      DCD_ASSERT(apply_idx < std::numeric_limits<uint32_t>::max(), wasm_serialization_error, "apply not exported");
      const vm::func_type& apply_type = bkend.get_module().get_function_type(apply_idx);
      DCD_ASSERT((apply_type == vm::host_function{{vm::i64, vm::i64, vm::i64}, {}}), wasm_serialization_error, "apply has wrong type");
   } catch(vm::exception& e) {
      DCD_THROW(wasm_serialization_error, e.detail());
   }
}
...
class dcd_vm_instantiated_module : public wasm_instantiated_module_interface {
   using backend_t = dcd_vm_backend_t<Impl>;
   public:

      dcd_vm_instantiated_module(dcd_vm_runtime<Impl>* runtime, std::unique_ptr<backend_t> mod) :
         _runtime(runtime),
      ...
      void apply(apply_context& context) override {
         ...
         try {
            ...
            FC_THROW_EXCEPTION(wasm_execution_error, "dcd-vm system failure");
         }
         _runtime->_bkend = nullptr;
      }

   private:
      dcd_vm_runtime<Impl>*            _runtime;
      std::unique_ptr<backend_t> _instantiated_module;
};

template<typename Impl>
dcd_vm_runtime<Impl>::dcd_vm_runtime() {}

template<typename Impl>
void dcd_vm_runtime<Impl>::immediately_exit_currently_running_module() {
   throw wasm_exit{};
}

template<typename Impl>
bool dcd_vm_runtime<Impl>::inject_module(IR::Module& module) {
   return false;
}

template<typename Impl>
std::unique_ptr<wasm_instantiated_module_interface> dcd_vm_runtime<Impl>::instantiate_module(const char* code_bytes, size_t code_size, std::vector<uint8_t>,
   ...
   try {
      ...
      dcd_vm_host_functions_t::resolve(bkend->get_module());
      return std::make_unique<dcd_vm_instantiated_module<Impl>>(this, std::move(bkend));
   } catch(dcd::vm::exception& e) {
      FC_THROW_EXCEPTION(wasm_execution_error, "Error building dcd-vm interp: ${e}", ("e", e.what()));
   }
}

template class dcd_vm_runtime<dcd::vm::interpreter>;
template class dcd_vm_runtime<dcd::vm::jit>;
...
struct host_function_registrator {
   ...
   constexpr host_function_registrator(Mod mod_name, Name fn_name) {
      using rhf_t = dcd_vm_host_functions_t;
      ...
      dcdvmoc::register_dcdvm_oc<HostFunction, is_injected, std::tuple<Preconditions...>>(
