...
#include <dcd/chain/webassembly/dcd-vm-oc/ipc_protocol.hpp>
#include <dcd/chain/webassembly/dcd-vm-oc/compile_monitor.hpp>
#include <dcd/chain/webassembly/dcd-vm-oc/ipc_helpers.hpp>
#include <dcd/chain/webassembly/dcd-vm-oc/compile_trampoline.hpp>
...
struct compile_monitor_session {
   ...
   void kick_compile_off(const code_tuple& code_id, wrapped_fd&& wasm_code) {
      ...
      dcdvmoc_message trampoline_compile_request = compile_wasm_message{code_id};
...
void launch_compile_monitor(int dcdnode_fd) {
...

   {
      ...
      if(monitor._compile_sessions.size())
         std::cerr << "ERROR: DCD VM OC compiler monitor exiting with active sessions" << std::endl;
   }
...
wrapped_fd get_connection_to_compile_monitor(int cache_fd) {
   FC_ASSERT(the_compile_monitor_trampoline.compile_manager_pid >= 0, "DCD VM oop connection doesn't look active");
   ...
   DCD_ASSERT(success, misc_exception, "failed to read response from monitor process");
   DCD_ASSERT(std::holds_alternative<initalize_response_message>(message), misc_exception, "unexpected response from monitor process");
   DCD_ASSERT(!std::get<initalize_response_message>(message).error_message, misc_exception, "Error message from monitor process: ${e}", ("e", *std::get<initalize_response_message>(message).error_message));
