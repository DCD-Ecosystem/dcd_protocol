...
#define DCDVMOC_MEMORY_PTR_cb_ptr GS_PTR struct dcd_vm_oc_control_block* const cb_ptr = ((GS_PTR struct dcd_vm_oc_control_block* const)(DCD_VM_OC_CONTROL_BLOCK_OFFSET));

int32_t dcd_vm_oc_grow_memory(int32_t grow, int32_t max) {
   DCDVMOC_MEMORY_PTR_cb_ptr;
   ...
   int64_t max_segments = cb_ptr->execution_thread_memory_length / DCD_VM_OC_MEMORY_STRIDE - 1;
   ...
   current_gs += gs_diff * DCD_VM_OC_MEMORY_STRIDE;
...
sigjmp_buf* dcd_vm_oc_get_jmp_buf() {
   DCDVMOC_MEMORY_PTR_cb_ptr;
   return cb_ptr->jmp;
}

void* dcd_vm_oc_get_exception_ptr() {
   DCDVMOC_MEMORY_PTR_cb_ptr;
   return cb_ptr->eptr;
}

void* dcd_vm_oc_get_bounce_buffer_list() {
   DCDVMOC_MEMORY_PTR_cb_ptr;
