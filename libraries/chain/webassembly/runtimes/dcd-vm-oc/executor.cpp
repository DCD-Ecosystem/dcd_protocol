...
#error DCD VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_code_start+cb_in_main_segment->execution_thread_code_length)
         siglongjmp(*cb_in_main_segment->jmp, DCDVMOC_EXIT_CHECKTIME_FAIL);
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, DCDVMOC_EXIT_SEGV);
...
static intrinsic grow_memory_intrinsic DCDVMOC_INTRINSIC_INIT_PRIORITY("dcdvmoc_internal.grow_memory", IR::FunctionType::get(IR::ResultType::i32,{IR::ValueType::i32,IR::ValueType::i32}),
  (void*)&dcd_vm_oc_grow_memory,
  std::integral_constant<std::size_t, find_intrinsic_index("dcdvmoc_internal.grow_memory")>::value
);

//This is effectively overriding the dcd_exit intrinsic in wasm_interface
static void dcd_exit(int32_t code) {
   siglongjmp(*dcd_vm_oc_get_jmp_buf(), DCDVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
...
static void throw_internal_exception(const char* const s) {
   *reinterpret_cast<std::exception_ptr*>(dcd_vm_oc_get_exception_ptr()) = std::make_exception_ptr(wasm_execution_error(FC_LOG_MESSAGE(error, s)));
   siglongjmp(*dcd_vm_oc_get_jmp_buf(), DCDVMOC_EXIT_EXCEPTION);
   __builtin_unreachable();
}

#define DEFINE_DCDVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function DCDVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
     std::integral_constant<std::size_t, find_intrinsic_index(#module "." #name)>::value \
   ); \
	void name()

DEFINE_DCDVMOC_TRAP_INTRINSIC(dcdvmoc_internal,depth_assert) {
   throw_internal_exception("Exceeded call depth maximum");
}

DEFINE_DCDVMOC_TRAP_INTRINSIC(dcdvmoc_internal,div0_or_overflow) {
   throw_internal_exception("Division by 0 or integer overflow trapped");
}

DEFINE_DCDVMOC_TRAP_INTRINSIC(dcdvmoc_internal,indirect_call_mismatch) {
   throw_internal_exception("Indirect call function type mismatch");
}

DEFINE_DCDVMOC_TRAP_INTRINSIC(dcdvmoc_internal,indirect_call_oob) {
   throw_internal_exception("Indirect call index out of bounds");
}

DEFINE_DCDVMOC_TRAP_INTRINSIC(dcdvmoc_internal,unreachable) {
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. DCD VM OC may not run correctly on this platform");
...
void executor::execute(const code_descriptor& code, memory& mem, apply_context& context) {
   ...
   DCD_ASSERT(code.starting_memory_pages <= (int)max_pages, wasm_execution_error, "Initial memory out of range");
...
//   context.trx_context.checktime(); //catch any expiration that might have occurred before setting up callback
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      ...
      case DCDVMOC_EXIT_CHECKTIME_FAIL:
//         context.trx_context.checktime();
         break;
      case DCDVMOC_EXIT_SEGV:
         DCD_ASSERT(false, wasm_execution_error, "access violation");
         break;
      case DCDVMOC_EXIT_EXCEPTION: //exception
