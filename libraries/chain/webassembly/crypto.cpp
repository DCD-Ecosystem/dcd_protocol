...
namespace dcd { namespace chain { namespace webassembly {
   ...
   void interface::assert_recover_key( legacy_ptr<const fc::sha256> digest,
      ...
      DCD_ASSERT(static_cast<unsigned>(s.which()) < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_signature_type,
        "Unactivated signature type used during assert_recover_key");
      DCD_ASSERT(static_cast<unsigned>(p.which()) < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_key_type,
        "Unactivated key type used when creating assert_recover_key");

      if(context.control.is_producing_block())
         DCD_ASSERT(s.variable_size() <= context.control.configured_subjective_signature_length_limit(),
                    sig_variable_size_limit_exception, "signature variable length component size greater than subjective maximum");

      auto check = fc::crypto::public_key( s, *digest, false );
      DCD_ASSERT( check == p, crypto_api_exception, "Error expected key different than recovered key" );
   }
   ...
   int32_t interface::recover_key( legacy_ptr<const fc::sha256> digest,
      ...
      if (static_cast<unsigned>(s.which()) >= config::genesis_num_supported_key_types ) {
         DCD_ASSERT(pub.size() >= 33, wasm_execution_error,
   ...
   void interface::assert_sha256(legacy_span<const char> data, legacy_ptr<const fc::sha256> hash_val) const {
      ...
      DCD_ASSERT( result == *hash_val, crypto_api_exception, "hash mismatch" );
   }
