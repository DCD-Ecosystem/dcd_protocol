...
abi_def dcd_contract_abi(const abi_def& dcd_system_abi);
vector<type_def> common_type_defs();

extern unsigned char dcd_abi_bin[2132];
...
ST& operator << (ST& s, const dcd::chain::may_not_exist<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
ST& operator >> (ST& s, dcd::chain::may_not_exist<T>& v) {
   if (s.remaining())
...
template<typename T>
void to_variant(const dcd::chain::may_not_exist<T>& e, fc::variant& v) {
   to_variant( e.value, v);
}

template<typename T>
void from_variant(const fc::variant& v, dcd::chain::may_not_exist<T>& e) {
   from_variant( v, e.value );
}


template<typename ST, typename T>
ST& operator << (ST& s, const dcd::chain::kv_tables_as_object<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
ST& operator >> (ST& s, dcd::chain::kv_tables_as_object<T>& v) {
   if( s.remaining() )
      raw::unpack(s, v.value);
   return s;
}
...
template<typename T>
void to_variant(const dcd::chain::kv_tables_as_object<T>& o, fc::variant& v) {
...
template<typename T>
void from_variant(const fc::variant& v, dcd::chain::kv_tables_as_object<T>& o) {
    EOS_ASSERT( v.is_object(), dcd::chain::invalid_type_inside_abi, "variant is not an variant_object type");
    ...
    for( const auto& table_it : tables ) {
        ...
        dcd::chain::kv_table_def kv_tbl_def;
        ...
        if( const auto st_it = table_obj.find("secondary_indices"); st_it != table_obj.end() ) {
            ...
            for( const auto& sidx_it : sec_indices_obj ) {
                dcd::chain::secondary_index_def idx_def;
                from_variant(sidx_it.value(), idx_def);
                kv_tbl_def.secondary_indices[dcd::chain::index_name(sidx_it.key())] = idx_def;
            }
        }
        kv_tables[dcd::chain::name(table_it.key())] = kv_tbl_def;
    }
}
} // namespace fc

FC_REFLECT( dcd::chain::type_def                         , (new_type_name)(type) )
FC_REFLECT( dcd::chain::field_def                        , (name)(type) )
FC_REFLECT( dcd::chain::struct_def                       , (name)(base)(fields) )
FC_REFLECT( dcd::chain::action_def                       , (name)(type)(ricardian_contract) )
FC_REFLECT( dcd::chain::table_def                        , (name)(index_type)(key_names)(key_types)(type) )
FC_REFLECT( dcd::chain::primary_index_def                , (name)(type) )
FC_REFLECT( dcd::chain::secondary_index_def              , (type) )
FC_REFLECT( dcd::chain::kv_table_def                     , (type)(primary_index)(secondary_indices) )
FC_REFLECT( dcd::chain::clause_pair                      , (id)(body) )
FC_REFLECT( dcd::chain::error_message                    , (error_code)(error_msg) )
FC_REFLECT( dcd::chain::variant_def                      , (name)(types) )
FC_REFLECT( dcd::chain::action_result_def                , (name)(result_type) )
FC_REFLECT( dcd::chain::abi_def                          , (version)(types)(structs)(actions)(tables)
