...
#include <dcd/chain/wasm_dcd_binary_ops.hpp>
...
namespace dcd { namespace chain { namespace wasm_injections {
   using namespace IR;
   ...
   struct injector_utils {
      ...
      static void add_import(Module& module, const char* func_name, int32_t& index ) {
         if (module.functions.imports.size() == 0 || registered_injected.find(func_name) == registered_injected.end() ) {
            ...
            decltype(module.functions.imports) new_import = { {{func_type_index}, dcd_injected_module_name, std::move(func_name)} };
...
#undef CLEAR
   ...
   struct call_depth_check_and_insert_checktime {
      ...
      static void init( Module& mod) {
         mod.globals.defs.push_back({{ValueType::i32, true}, {(I32)dcd::chain::wasm_constraints::maximum_call_depth}});
...
#undef INSERT_INJECTED
   ...
   struct f32_promote_injector {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         int32_t idx;
         injector_utils::add_import<ResultType::f64, ValueType::f32>( *(arg.module), u8"_dcd_f32_promote", idx );
   ...
   struct f64_demote_injector {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         int32_t idx;
         injector_utils::add_import<ResultType::f32, ValueType::f64>( *(arg.module), u8"_dcd_f64_demote", idx );
         wasm_ops::op_types<>::call_t f32promote;
         f32promote.field = idx;
         f32promote.pack(arg.new_code);
      }
      ...
      public:
         ...
         void inject() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::DCD_OperatorDecoderStream<std::conditional_t<full_injection, pre_op_full_injectors, pre_op_injectors>> pre_decoder(fd.code);
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::DCD_OperatorDecoderStream<std::conditional_t<full_injection, post_op_full_injectors, post_op_injectors>> post_decoder(fd.code);
...
}}} // namespace wasm_constraints, chain, dcd
