...
namespace dcd { namespace vm { class wasm_allocator; }}
   ...
   class controller {
      public:
         struct config {
            ...
            bool                     disable_all_subjective_mitigations = false; //< for developer & testing purposes, can be configured using `disable-all-subjective-mitigations` when `DCD_DEVELOPER` build option is provided
...
//         uint32_t get_greylist_limit()const;
      ...
      static std::optional<chain_id_type> extract_chain_id_from_db( const path& state_dir );
         ...
         dcd::chain::combined_database& kv_db()const;
