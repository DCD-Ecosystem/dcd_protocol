...
#include <dcd/chain/chain_config_helper.hpp>
...
FC_REFLECT(dcd::chain::chain_config_v0,
...
FC_REFLECT_DERIVED(dcd::chain::chain_config_v1, (dcd::chain::chain_config_v0), 
...
inline DataStream &operator<<(DataStream &s, const dcd::chain::data_entry<dcd::chain::chain_config_v0, dcd::chain::config_entry_validator> &entry){
   ...
   DCD_ASSERT(entry.is_allowed(), unsupported_feature, "config id ${id} is no allowed", ("id", entry.id));
 ...
 */
template <typename DataStream>
inline DataStream &operator<<(DataStream &s, const dcd::chain::data_entry<dcd::chain::chain_config_v1, dcd::chain::config_entry_validator> &entry){
   using namespace dcd::chain;
   ...
   //When the protocol feature is not activated, the old version of dcdnode that doesn't know about 
   //the entry MUST behave the same as the new version of dcdnode that does.
 ...
 */
template <typename DataStream>
inline DataStream &operator>>(DataStream &s, dcd::chain::data_entry<dcd::chain::chain_config_v0, dcd::chain::config_entry_validator> &entry){
   using namespace dcd::chain;

   DCD_ASSERT(entry.is_allowed(), dcd::chain::unsupported_feature, "config id ${id} is no allowed", ("id", entry.id));
   ...
   switch (entry.id){
      ...
      break;
      default:
      FC_THROW_EXCEPTION(dcd::chain::config_parse_error, "DataStream& operator<<: no such id: ${id}", ("id", entry.id));
   }
 ...
 */
template <typename DataStream>
inline DataStream &operator>>(DataStream &s, dcd::chain::data_entry<dcd::chain::chain_config_v1, dcd::chain::config_entry_validator> &entry){
   using namespace dcd::chain;
   ...
   switch (entry.id){
      ...
      break;
      default:
      dcd::chain::data_entry<chain_config_v0, config_entry_validator> base_entry(entry);
...
inline DataStream& operator<<( DataStream& s, const dcd::chain::data_range<T, dcd::chain::config_entry_validator>& selection ) {
   using namespace dcd::chain;
   ...
   for (auto uid : selection.ids){
      uint32_t id = uid;
      DCD_ASSERT(id < visited.size(), config_parse_error, "provided id ${id} should be less than ${size}", ("id", id)("size", visited.size()));
      DCD_ASSERT(!visited[id], config_parse_error, "duplicate id provided: ${id}", ("id", id));
      visited[id] = true;
 ...
 */
template<typename DataStream, typename T>
inline DataStream& operator>>( DataStream& s, dcd::chain::data_range<T, dcd::chain::config_entry_validator>& selection ) {
   using namespace dcd::chain;
   ...
   for (uint32_t i = 0; i < length; ++i) {
      ...
      DCD_ASSERT(id.value < visited.size(), config_parse_error, "provided id ${id} should be less than ${size}", ("id", id)("size", visited.size()));
      DCD_ASSERT(!visited[id], config_parse_error, "duplicate id provided: ${id}", ("id", id));
      visited[id] = true;
