...
struct asset : fc::reflect_init
{
   ...
   explicit asset(share_type a = 0, symbol id = symbol(CORE_SYMBOL)) :amount(a), sym(id) {
      DCD_ASSERT( is_amount_within_range(), asset_type_exception, "magnitude of asset amount must be less than 2^62" );
      DCD_ASSERT( sym.valid(), asset_type_exception, "invalid symbol" );
   }
   ...
   {
      DCD_ASSERT(get_symbol() == o.get_symbol(), asset_type_exception, "addition between two different asset is not allowed");
      amount += o.amount;
      return *this;
   }
   ...
   asset& operator -= (const asset& o)
   {
      DCD_ASSERT(get_symbol() == o.get_symbol(), asset_type_exception, "subtraction between two different asset is not allowed");
      amount -= o.amount;
      return *this;
   }
   ...
   friend bool operator < (const asset& a, const asset& b)
   {
      DCD_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "logical operation between two different asset is not allowed");
   ...
   friend asset operator - (const asset& a, const asset& b) {
      DCD_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "subtraction between two different asset is not allowed");
      return asset(a.amount - b.amount, a.get_symbol());
   }

   friend asset operator + (const asset& a, const asset& b) {
      DCD_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "addition between two different asset is not allowed");
      return asset(a.amount + b.amount, a.get_symbol());
   }
...
namespace fc {
inline void to_variant(const dcd::chain::asset& var, fc::variant& vo) { vo = var.to_string(); }
inline void from_variant(const fc::variant& var, dcd::chain::asset& vo) {
   vo = dcd::chain::asset::from_string(var.get_string());
}
}

namespace fc {
inline void from_variant(const fc::variant& var, dcd::chain::extended_asset& vo) {
   if( var.is_array() ) {
...
FC_REFLECT(dcd::chain::asset, (amount)(sym))
FC_REFLECT(dcd::chain::extended_asset, (quantity)(contract) )
