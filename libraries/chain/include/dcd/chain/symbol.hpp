...
#include <dcd/chain/core_symbol.hpp>
#include <string>
#include <functional>
...
namespace dcd {
   namespace chain {
      ...
      /**
         ...
         from_string constructs a symbol from an input a string of the form "4,DCD"
...
#define SY(P,X) ::dcd::chain::string_to_symbol_c(P,#X)
      ...
      static uint64_t string_to_symbol(uint8_t precision, const char* str) {
         try {
            ...
            for (uint32_t i = 0; i < len; ++i) {
               ...
               DCD_ASSERT (str[i] >= 'A' && str[i] <= 'Z', symbol_type_exception, "invalid character in symbol name");
      ...
      class symbol : fc::reflect_init {
         public:
            ...
            explicit symbol(uint8_t p, const char* s): m_value(string_to_symbol(p, s)) {
               DCD_ASSERT(valid(), symbol_type_exception, "invalid symbol: ${s}", ("s",s));
            }
            explicit symbol(uint64_t v = CORE_SYMBOL): m_value(v) {
               DCD_ASSERT(valid(), symbol_type_exception, "invalid symbol: ${name}", ("name",name()));
            }
            static symbol from_string(const string& from)
            {
               try {
                  string s = fc::trim(from);
                  DCD_ASSERT(!s.empty(), symbol_type_exception, "creating symbol from empty string");
                  auto comma_pos = s.find(',');
                  DCD_ASSERT(comma_pos != string::npos, symbol_type_exception, "missing comma in symbol");
                  auto prec_part = s.substr(0, comma_pos);
                  uint8_t p = fc::to_int64(prec_part);
                  string name_part = s.substr(comma_pos + 1);
                  DCD_ASSERT( p <= max_precision, symbol_type_exception, "precision ${p} should be <= 18", ("p", p));
            ...
            {
               DCD_ASSERT( decimals() <= max_precision, symbol_type_exception, "precision ${p} should be <= 18", ("p", decimals()) );
               uint64_t p10 = 1;
               uint64_t p = decimals();
               while( p > 0  ) {
                  p10 *= 10; --p;
            ...
            explicit operator string() const
            {
               uint64_t v = m_value;
               uint8_t p = v & 0xFF;
               string ret = dcd::chain::to_string(p);
               ret += ',';
               ret += name();
               return ret;
            }
            ...
            void reflector_init()const {
               DCD_ASSERT( decimals() <= max_precision, symbol_type_exception, "precision ${p} should be <= 18", ("p", decimals()) );
               DCD_ASSERT( valid_name(name()), symbol_type_exception, "invalid symbol: ${name}", ("name",name()));
            }
...
namespace fc {
   inline void to_variant(const dcd::chain::symbol& var, fc::variant& vo) { vo = var.to_string(); }
   inline void from_variant(const fc::variant& var, dcd::chain::symbol& vo) {
      vo = dcd::chain::symbol::from_string(var.get_string());
   }
}

namespace fc {
   inline void to_variant(const dcd::chain::symbol_code& var, fc::variant& vo) {
      vo = dcd::chain::symbol(var.value << 8).name();
   }
   inline void from_variant(const fc::variant& var, dcd::chain::symbol_code& vo) {
      vo = dcd::chain::symbol(0, var.get_string().c_str()).to_symbol_code();
   }
}

FC_REFLECT(dcd::chain::symbol_code, (value))
FC_REFLECT(dcd::chain::symbol, (m_value))
FC_REFLECT(dcd::chain::extended_symbol, (sym)(contract))
