...
namespace fc {
  class variant;
  void to_variant(const dcd::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, dcd::chain::name& check);
} // fc
...
namespace dcd::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, dcd::chain::name& check);
...
namespace std {
   template<> struct hash<dcd::chain::name> : private hash<uint64_t> {
      typedef dcd::chain::name argument_type;
...
FC_REFLECT( dcd::chain::name, (value) )
