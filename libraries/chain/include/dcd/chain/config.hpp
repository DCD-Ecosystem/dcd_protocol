...
namespace dcd { namespace chain { namespace config {
...
const static name system_account_name    { "dcd"_n };
const static name null_account_name      { "dcd.null"_n };
const static name producers_account_name { "dcd.prods"_n };
...
const static name dcd_auth_scope       { "dcd.auth"_n };
const static name dcd_all_scope        { "dcd.all"_n };

const static name active_name     { "active"_n };
const static name owner_name      { "owner"_n };
const static name dcd_any_name  { "dcd.any"_n };
const static name dcd_code_name { "dcd.code"_n };
...
const static dcd::chain::wasm_interface::vm_type default_wasm_runtime = dcd::chain::wasm_interface::vm_type::dcd_vm_jit;
...
} } } // namespace dcd::chain::config

constexpr uint64_t DCD_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / dcd::chain::config::percent_100;
}

template<typename Number>
Number DCD_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + dcd::chain::config::percent_100 - dcd::chain::config::percent_1)  / dcd::chain::config::percent_100;
}
