...
FC_REFLECT( dcd::chain::detail::block_header_state_common,
            (block_num)
...
FC_REFLECT( dcd::chain::detail::schedule_info,
            (schedule_lib_num)
            (schedule_hash)
            (schedule)
)

FC_REFLECT( dcd::chain::detail::rate_info,
            (rate_lib_num)
            (rate_hash)
            (rate_info)
)
...
// @ignore header_exts
FC_REFLECT_DERIVED(  dcd::chain::block_header_state, (dcd::chain::detail::block_header_state_common),
                     (id)
                     (header)
                     (pending_schedule)
                     (pending_rate)
...
FC_REFLECT( dcd::chain::legacy::snapshot_block_header_state_v2::schedule_info,
          ( schedule_lib_num )
          ( schedule_hash )
          ( schedule )
)
...
FC_REFLECT( dcd::chain::legacy::snapshot_block_header_state_v2,
          ( block_num )
          ( dpos_proposed_irreversible_blocknum )
          ( dpos_irreversible_blocknum )
          ( active_schedule )
