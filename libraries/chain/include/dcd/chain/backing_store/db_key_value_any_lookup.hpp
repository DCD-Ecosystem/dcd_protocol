...
namespace dcd { namespace chain { namespace backing_store {
   ...
   struct key_bundle {
      ...
      dcd::session::shared_bytes full_key;
   };
   ...
   struct prefix_bundle {
      ...
      dcd::session::shared_bytes full_key;
      dcd::session::shared_bytes prefix_key;
   };

   template<typename Key>
   struct value_bundle : Key {
      value_bundle(Key&& key, std::optional<dcd::session::shared_bytes>&& v) : Key(std::move(key)), value(std::move(v)) {}
      value_bundle(const Key& key, std::optional<dcd::session::shared_bytes>&& v) : Key(key), value(std::move(v)) {}
      std::optional<dcd::session::shared_bytes> value;
   };
