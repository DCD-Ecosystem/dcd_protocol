...
namespace dcd { namespace chain { namespace backing_store {
   ...
   struct secondary_helper<uint64_t> {
      ...
      dcd::session::shared_bytes value(const uint64_t& sec_key, name payer) {
   ...
   template<>
   struct secondary_helper<dcd::chain::uint128_t> {
      const char* desc() { return "idx128"; }
      dcd::session::shared_bytes value(const dcd::chain::uint128_t& sec_key, name payer) {
         return payer_payload(payer, nullptr, 0).as_payload();
      }
   ...
   template<>
   struct secondary_helper<dcd::chain::key256_t> {
      const char* desc() { return "idx256"; }
      dcd::session::shared_bytes value(const dcd::chain::key256_t& sec_key, name payer) {
         return payer_payload(payer, nullptr, 0).as_payload();
      }
   ...
   struct secondary_helper<float64_t> {
      ...
      dcd::session::shared_bytes value(const float64_t& sec_key, name payer) {
   ...
   struct secondary_helper<float128_t> {
      ...
      dcd::session::shared_bytes value(const float128_t& sec_key, name payer) {
         constexpr static auto value_size = sizeof(sec_key);
