...
template <typename F>
bool read_rocksdb_entry(const dcd::session::shared_bytes& actual_db_kv_key,
                        const dcd::session::shared_bytes& value,
                        F& function) {
   uint64_t    contract;
...
namespace detail {
   ...
   class iterator_pair {
   public:
      iterator_pair(const dcd::session::shared_bytes& begin_key,
...
void walk_rocksdb_entries_with_prefix(const kv_undo_stack_ptr& kv_undo_stack,
                                      const dcd::session::shared_bytes& begin_key,
                                      const dcd::session::shared_bytes& end_key,
                                      F& function) {
...
bool process_rocksdb_entry(Session& session,
                           const dcd::session::shared_bytes& key,
                           Receiver& receiver) {
   if (!key) {
      return false;
   }
...
const char* contract_table_type() {
   ...
   else if constexpr (std::is_same_v<Object, secondary_index_view<dcd::chain::uint128_t>>) {
      return "secondary_index_view<uint128_t>";
   }
   else if constexpr (std::is_same_v<Object, secondary_index_view<dcd::chain::key256_t>>) {
...
FC_REFLECT(dcd::chain::backing_store::table_id_object_view, (code)(scope)(table)(payer)(count) )
FC_REFLECT(dcd::chain::backing_store::primary_index_view, (primary_key)(payer)(value) )
REFLECT_SECONDARY(dcd::chain::backing_store::secondary_index_view<uint64_t>)
REFLECT_SECONDARY(dcd::chain::backing_store::secondary_index_view<dcd::chain::uint128_t>)
REFLECT_SECONDARY(dcd::chain::backing_store::secondary_index_view<dcd::chain::key256_t>)
REFLECT_SECONDARY(dcd::chain::backing_store::secondary_index_view<float64_t>)
REFLECT_SECONDARY(dcd::chain::backing_store::secondary_index_view<float128_t>)

namespace fc {
   inline
   void to_variant( const dcd::chain::backing_store::blob& b, variant& v ) {
      v = variant(base64_encode(b.str_data.data(), b.str_data.size()));
   }

   inline
   void from_variant( const variant& v, dcd::chain::backing_store::blob& b ) {
