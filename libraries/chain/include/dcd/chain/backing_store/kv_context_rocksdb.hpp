...
namespace dcd { namespace chain {
   ...
   // to another payer
   inline static dcd::session::shared_bytes build_value(const char* value, uint32_t value_size, const account_name& payer) {
      const uint32_t final_value_size = backing_store::payer_in_value_size + value_size;
      auto result = dcd::session::shared_bytes(final_value_size);
   ...
   static inline dcd::session::shared_bytes make_prefix_key(uint64_t contract, const char* user_prefix, uint32_t user_prefix_size) {
      static auto rocks_prefix = make_rocksdb_contract_kv_prefix();

      auto result = dcd::session::shared_bytes(rocks_prefix.size() + sizeof(contract) + user_prefix_size);
   ...
   static inline dcd::session::shared_bytes
   make_composite_key(uint64_t contract, const char* user_prefix, uint32_t user_prefix_size, const char* key, uint32_t key_size) {
      static auto rocks_prefix = make_rocksdb_contract_kv_prefix(); 

      auto result = dcd::session::shared_bytes(rocks_prefix.size() + sizeof(contract) + user_prefix_size + key_size);
      std::memcpy(result.data(), rocks_prefix.data(), rocks_prefix.size());
      b1::chain_kv::insert_key(result, rocks_prefix.size(), contract);
      std::memcpy(result.data() + rocks_prefix.size() + sizeof(contract), user_prefix, user_prefix_size);
   ...
   static inline dcd::session::shared_bytes make_composite_key(const dcd::session::shared_bytes& prefix,
                                                                 const char* key, uint32_t key_size) {
      auto result = dcd::session::shared_bytes(prefix.size() + key_size);
      std::memcpy(result.data(), prefix.data(), prefix.size());
      std::memcpy(result.data() + prefix.size(), key, key_size);
      return result;
   }

   static inline int32_t compare_bytes(const dcd::session::shared_bytes& left, const dcd::session::shared_bytes& right) {
      if (left < right) {
          return -1;
      }
   ...
   struct kv_iterator_rocksdb : kv_iterator {
      ...
      dcd::session::shared_bytes    kv_prefix; // Format: [contract, prefix]
   ...
   struct kv_context_rocksdb : kv_context {
      ...
      dcd::session::shared_bytes                current_key;
      std::optional<dcd::session::shared_bytes> current_value;
      ...
      int64_t kv_erase(uint64_t contract, const char* key, uint32_t key_size) override {
         ...
         auto old_value             = std::optional<dcd::session::shared_bytes>{};
