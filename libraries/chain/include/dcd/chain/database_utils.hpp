...
namespace fc {
   ...
   inline
   void float128_to_uint128 (const float128_t& f, dcd::chain::uint128_t& u) {
      memcpy(&u, &f, sizeof(u));
   }

   inline
   void uint128_to_float128 (const dcd::chain::uint128_t& u,  float128_t& f) {
      memcpy(&f, &u, sizeof(f));
   }
   ...
   void to_variant( const float128_t& f, variant& v ) {
      ...
      char as_bytes[sizeof(dcd::chain::uint128_t)];
   ...
   void from_variant( const variant& v, float128_t& f ) {
      ...
      char temp[sizeof(dcd::chain::uint128_t)];
   ...
   inline
   void to_variant( const dcd::chain::shared_string& s, variant& v ) {
      v = variant(std::string(s.begin(), s.end()));
   }

   inline
   void from_variant( const variant& v, dcd::chain::shared_string& s ) {
      string _s;
      from_variant(v, _s);
      s = dcd::chain::shared_string(_s.begin(), _s.end(), s.get_allocator());
   }

   inline
   void to_variant( const dcd::chain::shared_blob& b, variant& v ) {
      v = variant(base64_encode(b.data(), b.size()));
   }

   inline
   void from_variant( const variant& v, dcd::chain::shared_blob& b ) {
      string _s = base64_decode(v.as_string());
      b = dcd::chain::shared_blob(_s.begin(), _s.end(), b.get_allocator());
   }
   ...
   template<typename T>
   void to_variant( const dcd::chain::shared_vector<T>& sv, variant& v ) {
      to_variant(std::vector<T>(sv.begin(), sv.end()), v);
   }

   template<typename T>
   void from_variant( const variant& v, dcd::chain::shared_vector<T>& sv ) {
      std::vector<T> _v;
      from_variant(v, _v);
      sv = dcd::chain::shared_vector<T>(_v.begin(), _v.end(), sv.get_allocator());
   }
}
