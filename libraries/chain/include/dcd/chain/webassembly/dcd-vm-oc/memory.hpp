...
#include <dcd/chain/webassembly/dcd-vm-oc/gs_seg_helpers.h>
...
class memory {
      ...
      static constexpr uint64_t mutable_global_size               = 8u  * dcd::chain::wasm_constraints::maximum_mutable_globals/4u;
      static constexpr uint64_t table_size                        = 16u * dcd::chain::wasm_constraints::maximum_table_elements;
      static constexpr size_t   wcb_allowance                     = 512u;
      static_assert(sizeof(control_block) <= wcb_allowance, "DCD VM OC memory doesn't set aside enough memory for control block");
   ...
   public:
      ...
      static_assert(-cb_offset == DCD_VM_OC_CONTROL_BLOCK_OFFSET, "DCD VM OC control block offset has slid out of place somehow");
      static_assert(stride == DCD_VM_OC_MEMORY_STRIDE, "DCD VM OC memory stride has slid out of place somehow");
...
#define OFFSET_OF_CONTROL_BLOCK_MEMBER(M) (-(int)dcd::chain::dcdvmoc::memory::cb_offset + (int)offsetof(dcd::chain::dcdvmoc::control_block, M))
#define OFFSET_OF_FIRST_INTRINSIC ((int)-dcd::chain::dcdvmoc::memory::first_intrinsic_offset)
