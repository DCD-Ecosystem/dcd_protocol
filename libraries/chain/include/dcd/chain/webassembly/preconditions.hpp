...
namespace dcd { namespace chain { namespace webassembly {
   namespace detail {
      ...
      struct is_whitelisted_type<vm::span<T>> {
         ...
         // If we wish to expand to general span<T> in the future, changes are needed in DCD VM
   ...
   DCD_VM_PRECONDITION(context_free_check,
         DCD_VM_INVOKE_ONCE([&](auto&&...) {
            DCD_ASSERT(ctx.get_host().get_context().is_context_free(), unaccessible_api, "this API may only be called from context_free apply");
         }));

   DCD_VM_PRECONDITION(context_aware_check,
         DCD_VM_INVOKE_ONCE([&](auto&&...) {
            DCD_ASSERT(!ctx.get_host().get_context().is_context_free(), unaccessible_api, "only context free api's can be used in this context");
         }));

   DCD_VM_PRECONDITION(privileged_check,
         DCD_VM_INVOKE_ONCE([&](auto&&...) {
            DCD_ASSERT(ctx.get_host().get_context().is_privileged(), unaccessible_api,
   ...
   DCD_VM_PRECONDITION(core_precondition,
         DCD_VM_INVOKE_ON_ALL(([&](auto&& arg, auto&&... rest) {
            ...
            if constexpr (is_span_type_v<arg_t> || vm::is_argument_proxy_type_v<arg_t>) {
               dcd::vm::invoke_on<false, dcd::vm::invoke_on_all_t>([&arg](auto&& narg, auto&&... nrest) {
                  using nested_arg_t = std::decay_t<decltype(narg)>;
                  if constexpr (dcd::vm::is_span_type_v<nested_arg_t> || vm::is_argument_proxy_type_v<nested_arg_t>)
                      DCD_ASSERT(!is_aliasing(detail::to_span(arg), detail::to_span(narg)), wasm_exception, "pointers not allowed to alias");
               }, rest...);
            }
         })));
   ...
   DCD_VM_PRECONDITION(is_nan_check,
         DCD_VM_INVOKE_ON_ALL([&](auto&& arg, auto&&... rest) {
            if constexpr (should_check_nan_v<std::remove_cv_t<typename remove_argument_proxy<std::decay_t<decltype(arg)>>::type>>) {
               DCD_ASSERT(!webassembly::is_nan(*arg), transaction_exception, "NaN is not an allowed value for a secondary key");
            }
         }));

   DCD_VM_PRECONDITION(legacy_static_check_wl_args,
         DCD_VM_INVOKE_ONCE([&](auto&&... args) {
