...
#include <dcd/chain/webassembly/dcd-vm-oc/executor.hpp>
...
using namespace dcd::chain::dcdvmoc;

class dcdvmoc_instantiated_module;

class dcdvmoc_runtime : public dcd::chain::wasm_runtime_interface {
   public:
      dcdvmoc_runtime(const boost::filesystem::path data_dir, const dcdvmoc::config& dcdvmoc_config, const chainbase::database& db);
      ~dcdvmoc_runtime();
      ...
      friend dcdvmoc_instantiated_module;
      dcdvmoc::code_cache_sync cc;
...
struct dcd_vm_oc_execution_interface {
   inline const auto& operand_from_back(std::size_t index) const { return *(os - index - 1); }
   dcd::vm::native_value* os;
};

struct dcd_vm_oc_type_converter : public dcd::vm::type_converter<webassembly::interface, dcd_vm_oc_execution_interface> {
   using base_type = dcd::vm::type_converter<webassembly::interface, dcd_vm_oc_execution_interface>;
   ...
   DCD_VM_FROM_WASM(memcpy_params, (vm::wasm_ptr_t dst, vm::wasm_ptr_t src, vm::wasm_size_t size)) {
   ...
   DCD_VM_FROM_WASM(memcmp_params, (vm::wasm_ptr_t lhs, vm::wasm_ptr_t rhs, vm::wasm_size_t size)) {
     auto l = array_ptr_impl<char>(lhs, size);
     auto r = array_ptr_impl<char>(rhs, size);
     return { l, r, size };
   }

   DCD_VM_FROM_WASM(memset_params, (vm::wasm_ptr_t dst, int32_t val, vm::wasm_size_t size)) {
     auto d = array_ptr_impl<char>(dst, size);
     array_ptr_impl<char>(dst, 1);
     return { d, val, size };
   }
   ...
   auto from_wasm(vm::wasm_ptr_t ptr, vm::tag<T> = {}) const
      ...
      if constexpr(T::is_legacy()) {
         DCD_ASSERT(ptr != 0, wasm_execution_error, "references cannot be created for null pointers");
      }
      void* p = array_ptr_impl<typename T::pointee_type>(ptr, 1);
      return {p};
   }

   DCD_VM_FROM_WASM(null_terminated_ptr, (vm::wasm_ptr_t ptr)) {
...
template<typename Args, std::size_t... Is>
auto get_ct_args(std::index_sequence<Is...>) {
   return std::tuple_cat(get_ct_args_i<dcd_vm_oc_type_converter, std::tuple_element_t<Is, Args>>()...);
}

struct result_resolver {
   // Suppress "expression result unused" warnings
   result_resolver(dcd_vm_oc_type_converter& tc) : tc(tc) {}
   template<typename T>
   auto operator,(T&& res) {
      return make_native_type(vm::detail::resolve_result(tc, static_cast<T&&>(res)));
   }
   dcd_vm_oc_type_converter& tc;
};
...
auto fn(A... a) {
   try {
      if constexpr(!is_injected) {
         ...
         constexpr int depth_assertion_intrinsic_offset = OFFSET_OF_FIRST_INTRINSIC - (int) find_intrinsic_index("dcdvmoc_internal.depth_assert") * 8;
      ...
      Interface host(*ctx);
      dcd_vm_oc_type_converter tc{&host, dcd_vm_oc_execution_interface{stack + sizeof...(A)}};
      return result_resolver{tc}, dcd::vm::invoke_with_host<F, Preconditions, native_args>(tc, &host, std::make_index_sequence<sizeof...(A)>());
   }
   catch(...) {
      *reinterpret_cast<std::exception_ptr*>(dcd_vm_oc_get_exception_ptr()) = std::current_exception();
   }
   siglongjmp(*dcd_vm_oc_get_jmp_buf(), DCDVMOC_EXIT_EXCEPTION);
...
void register_dcdvm_oc(Name n) {
   // Has special handling
   if(n == BOOST_HANA_STRING("env.dcd_exit")) return;
