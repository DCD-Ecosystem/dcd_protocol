...
#define DCD_VM_OPEN_NAMESPACE namespace dcd::vm {
#define DCD_VM_CLOSE_NAMESPACE }

#include "error_codes_pp.hpp"

// create a category for parser error codes
GENERATE_ERROR_CATEGORY(parser_errors, "dcd-vm parser errors")
GENERATE_ERROR_CATEGORY(memory_errors, "dcd-vm memory errors")
GENERATE_ERROR_CATEGORY(system_errors, "dcd-vm system errors")
GENERATE_ERROR_CATEGORY(auxiliary_errors, "dcd-vm auxiliary errors")
...
#define _DCD_VM_ASSERT(expr, err_type)                                                                                  \
   if (!UNLIKELY(expr)) {                                                                                              \
      return err_type;                                                                                                 \
   }

// assert but invalidate the enclosing class
#define DCD_VM_ASSERT_INVALIDATE(expr, err_type)                                                                       \
