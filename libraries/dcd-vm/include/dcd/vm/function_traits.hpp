...
#define DCD_VM_HAS_MEMBER(ARG, NAME)                           \
   dcd::vm::detail::overloaded {                             \
        [](auto&& f, std::enable_if_t<std::is_class_v<std::decay_t<decltype(f)>> && \
                        dcd::vm::detail::pass_type<          \
                            decltype(&std::decay_t<decltype(f)>::type::NAME)>(), int> = 0) constexpr { \
                            return true;                       \
       }, [](...) constexpr { return false; }                  \
    }(dcd::vm::detail::wrapper_t<decltype(ARG)>{})

#define DCD_VM_HAS_MEMBER_TY(TY, NAME)                         \
   dcd::vm::detail::overloaded {                             \
    ...
    }(dcd::vm::detail::wrapper_t<TY>{})

#define DCD_VM_HAS_TEMPLATE_MEMBER(ARG, NAME)                  \
   dcd::vm::detail::overloaded {                             \
        [&](auto&& f, std::enable_if_t<std::is_class_v<std::decay_t<decltype(f)>> && \
                        dcd::vm::detail::pass_type<          \
...
#define DCD_VM_HAS_TEMPLATE_MEMBER_TY(TY, NAME)                \
   dcd::vm::detail::overloaded {                             \
        [](auto&& f, std::enable_if_t<std::is_class_v<TY> &&   \
                        dcd::vm::detail::pass_type<          \
                            decltype(&std::decay_t<decltype(f)>::type::template NAME)>(), int> = 0) constexpr { \
...
#define AUTO_PARAM_WORKAROUND(X) dcd::vm::detail::make_dependent<decltype(X)>(X)
...
namespace dcd { namespace vm {
   ...
   template <auto FN>
   inline constexpr static bool is_callable_v = DCD_VM_HAS_MEMBER(AUTO_PARAM_WORKAROUND(FN), operator());

   template <typename F>
   constexpr bool is_callable(F&& fn) { return DCD_VM_HAS_MEMBER(fn, operator()); }
