...
#include <dcd/vm/function_traits.hpp>
...
namespace dcd { namespace vm {
   ...
   struct running_context {
      ...
      inline void validate_pointer(const void* ptr, wasm_size_t len) const {
         DCD_VM_ASSERT( len <= std::numeric_limits<wasm_size_t>::max() / (wasm_size_t)sizeof(T), wasm_interpreter_exception, "length will overflow" );
...
#define DCD_VM_FROM_WASM_ADD_TAG(...) (__VA_ARGS__, ::dcd::vm::tag<T> = {})

#define DCD_VM_FROM_WASM(TYPE, PARAMS) \
   template <typename T>                    \
   auto from_wasm DCD_VM_FROM_WASM_ADD_TAG PARAMS const -> std::enable_if_t<std::is_same_v<T, TYPE>, TYPE>
   ...
   namespace detail {
      ...
      template <typename S, typename Type_Converter>
      constexpr inline static bool has_from_wasm_v = DCD_VM_HAS_TEMPLATE_MEMBER_TY(Type_Converter, from_wasm<S>);
...
#define DCD_VM_INVOKE_ON(TYPE, CONDITION) \
   dcd::vm::invoke_on<false, TYPE>(CONDITION, args...);

#define DCD_VM_INVOKE_ON_ALL(CONDITION) \
   dcd::vm::invoke_on<false, dcd::vm::invoke_on_all_t>(CONDITION, args...);

#define DCD_VM_INVOKE_ONCE(CONDITION) \
   dcd::vm::invoke_on<true, dcd::vm::invoke_on_all_t>(CONDITION, args...);

#define DCD_VM_PRECONDITION(NAME, ...)                                       \
   struct NAME {                                                             \
   ...
   struct registered_host_functions {
      ...
      static void resolve(Module& mod) {
         ...
         for (std::size_t i = 0; i < mod.imports.size(); i++) {
            ...
            DCD_VM_ASSERT(current_mappings.named_mapping.count({ mod_name, fn_name }), wasm_link_exception,
            ...
            DCD_VM_ASSERT(entry.kind == Function, wasm_link_exception, "importing non-function");
            DCD_VM_ASSERT(current_mappings.host_functions[imports[i]] == mod.types[entry.type.func_t], wasm_link_exception, "wrong type for imported function");
         }
      }
