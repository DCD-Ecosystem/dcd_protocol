...
/* clang-format off */
#define DCD_VM_CONTROL_FLOW_OPS(opcode_macro)   \
...
#define DCD_VM_BR_TABLE_OP(opcode_macro)        \
   opcode_macro(br_table, 0x0E)
#define DCD_VM_RETURN_OP(opcode_macro)          \
   opcode_macro(return_, 0x0F)
#define DCD_VM_CALL_OPS(opcode_macro)           \
   opcode_macro(call, 0x10)                     \
   opcode_macro(call_indirect, 0x11)            \
   opcode_macro(br_table_data, 0x12)
#define DCD_VM_CALL_IMM_OPS(opcode_macro)       \
...
#define DCD_VM_PARAMETRIC_OPS(opcode_macro)     \
...
#define DCD_VM_VARIABLE_ACCESS_OPS(opcode_macro)\
...
#define DCD_VM_MEMORY_OPS(opcode_macro)         \
...
#define DCD_VM_I32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i32_const, 0x41)
#define DCD_VM_I64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i64_const, 0x42)
#define DCD_VM_F32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f32_const, 0x43)
#define DCD_VM_F64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f64_const, 0x44)
#define DCD_VM_COMPARISON_OPS(opcode_macro)     \
...
#define DCD_VM_NUMERIC_OPS(opcode_macro)        \
...
#define DCD_VM_CONVERSION_OPS(opcode_macro)     \
...
#define DCD_VM_EXIT_OP(opcode_macro)            \
   opcode_macro(exit, 0xC0)
#define DCD_VM_EMPTY_OPS(opcode_macro)          \
...
#define DCD_VM_ERROR_OPS(opcode_macro)          \
   opcode_macro(error, 0xFF)

/* clang-format on */

#define DCD_VM_CREATE_ENUM(name, code) name = code,

#define DCD_VM_CREATE_STRINGS(name, code) #name,

#define DCD_VM_CREATE_MAP(name, code) { code, #name },

#define DCD_VM_OPCODE_NAME_if_
#define DCD_VM_OPCODE_NAME_else_
#define DCD_VM_OPCODE_NAME_return_

#define DCD_VM_OPCODE_NAME_TEST() 1
#define DCD_VM_OPCODE_NAME_TEST_DCD_VM_OPCODE_NAME_TEST 0,
#define DCD_VM_OPCODE_NAME_TEST_1 1, ignore
#define DCD_VM_EXPAND(x) x
#define DCD_VM_CAT2(x, y) x ## y
#define DCD_VM_CAT(x, y) DCD_VM_CAT2(x, y)
#define DCD_VM_APPLY(f, args) f args
#define DCD_VM_FIX_OPCODE_NAME_0(name) name ## _t
#define DCD_VM_FIX_OPCODE_NAME_1(name) name ## t
#define DCD_VM_FIX_OPCODE_NAME(iskeyword, garbage) DCD_VM_FIX_OPCODE_NAME_ ## iskeyword

#define DCD_VM_OPCODE_T(name)                                                             \
   DCD_VM_APPLY(DCD_VM_FIX_OPCODE_NAME,                                                   \
      (DCD_VM_CAT(DCD_VM_OPCODE_NAME_TEST_,                                               \
           DCD_VM_EXPAND(DCD_VM_OPCODE_NAME_TEST DCD_VM_OPCODE_NAME_ ## name ()))))(name)

#define DCD_VM_CREATE_EXIT_TYPE(name, code)                                                                      \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t pc;                                                                                                     \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define DCD_VM_CREATE_CONTROL_FLOW_TYPES(name, code)                                                                   \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() {}                                                                                       \
      DCD_VM_OPCODE_T(name)(uint32_t data) : data(data) {}                                                             \
      DCD_VM_OPCODE_T(name)(uint32_t d, uint32_t pc, uint16_t i, uint16_t oi)                                          \
...
#define DCD_VM_CREATE_BR_TABLE_TYPE(name, code)                                                                        \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
...
#define DCD_VM_CREATE_TYPES(name, code)                                                                                \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define DCD_VM_CREATE_CALL_TYPES(name, code)                                                                           \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define DCD_VM_CREATE_CALL_IMM_TYPES(name, code)                                                                       \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      uint16_t locals;                                                                                                 \
...
#define DCD_VM_CREATE_VARIABLE_ACCESS_TYPES(name, code)                                                                \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define DCD_VM_CREATE_MEMORY_TYPES(name, code)                                                                         \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
...
#define DCD_VM_CREATE_I32_CONSTANT_TYPE(name, code)                                                                    \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
      explicit DCD_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit DCD_VM_OPCODE_T(name)(int32_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         int32_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define DCD_VM_CREATE_I64_CONSTANT_TYPE(name, code)                                                                    \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
      explicit DCD_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit DCD_VM_OPCODE_T(name)(int64_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         int64_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define DCD_VM_CREATE_F32_CONSTANT_TYPE(name, code)                                                                    \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
      explicit DCD_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit DCD_VM_OPCODE_T(name)(float n) { data.f = n; }                                                          \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         float    f;                                                                                                   \
      } data;                                                                                                          \
...
#define DCD_VM_CREATE_F64_CONSTANT_TYPE(name, code)                                                                    \
   struct DCD_VM_OPCODE_T(name) {                                                                                      \
      DCD_VM_OPCODE_T(name)() = default;                                                                               \
      explicit DCD_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit DCD_VM_OPCODE_T(name)(double n) { data.f = n; }                                                         \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         double   f;                                                                                                   \
      } data;                                                                                                          \
...
#define DCD_VM_IDENTITY(name, code) dcd::vm::DCD_VM_OPCODE_T(name),
#define DCD_VM_IDENTITY_END(name, code) dcd::vm::DCD_VM_OPCODE_T(name)
