...
DCD_REFLECT(microseconds, _count);
DCD_COMPARE(microseconds);
...
DCD_REFLECT(time_point, elapsed);
DCD_COMPARE(time_point);

template <typename S>
void from_json(time_point& obj, S& stream) {
   auto s = stream.get_string();
   uint64_t utc_microseconds;
   if (!dcd::string_to_utc_microseconds(utc_microseconds, s.data(), s.data() + s.size())) {
      check(false, convert_json_error(dcd::from_json_error::expected_time_point));
   }
...
void to_json(const time_point& obj, S& stream) {
   return to_json(dcd::microseconds_to_str(obj.elapsed._count), stream);
}
...
class time_point_sec {
 public:
   ...
   static time_point_sec min() { return time_point_sec(0); }
            ...
            operator time_point() const { return time_point(dcd::seconds(utc_seconds)); }
   uint32_t sec_since_epoch() const { return utc_seconds; }

   /// @cond INTERNAL
   time_point_sec operator=(const dcd::time_point& t) {
...
DCD_REFLECT(time_point_sec, utc_seconds);
DCD_COMPARE(time_point);
...
void from_json(time_point_sec& obj, S& stream) {
   auto s = stream.get_string();
   const char* p = s.data();
   if (!dcd::string_to_utc_seconds(obj.utc_seconds, p, s.data() + s.size(), true, true)) {
...
void to_json(const time_point_sec& obj, S& stream) {
   return to_json(dcd::microseconds_to_str(uint64_t(obj.utc_seconds) * 1'000'000), stream);
}
...
class block_timestamp {
 public:
   ...
   block_timestamp next() const {
      dcd::check(std::numeric_limits<uint32_t>::max() - slot >= 1, "block timestamp overflow");
...
DCD_REFLECT(block_timestamp_type, slot);
