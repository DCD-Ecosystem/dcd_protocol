...
void dcd_assert_message(uint32_t, const char*, uint32_t);
__attribute__((dcd_wasm_import, noreturn))
void dcd_assert(uint32_t, const char*);
__attribute__((dcd_wasm_import, noreturn))
void dcd_assert_code(uint32_t, uint64_t);
}
}
}
#else
 ...
 *  @ingroup core
 *  @brief Defines wrappers over dcd_assert
 */


struct dcd_error : std::exception {
   explicit dcd_error(uint64_t code) {}
};

namespace detail {
   [[noreturn]] inline void assert_or_throw(std::string_view msg) {
#ifdef __dcd_cdt__
         internal_use_do_not_use::dcd_assert_message(false, msg.data(), msg.size());
#else
...
#ifdef __dcd_cdt__
         internal_use_do_not_use::dcd_assert(false, msg);
#else
...
#ifdef __dcd_cdt__
         internal_use_do_not_use::dcd_assert_message(false, msg.c_str(), msg.size());
#else
...
#ifdef __dcd_cdt__
         internal_use_do_not_use::dcd_assert_code(false, code);
#else
 ...
 *  Example:
 *  @code
 *  dcd::check(a == b, "a does not equal b");
 *  @endcode
 */
inline void check(bool pred, std::string_view msg) {
   if (!pred)
      dcd::detail::assert_or_throw(msg);
}
...
inline void check(bool pred, const std::string& msg) {
   if (!pred)
      dcd::detail::assert_or_throw(std::string_view{msg.c_str(), msg.size()});
}
...
inline void check(bool pred, std::string&& msg) {
   if (!pred)
      dcd::detail::assert_or_throw(std::move(msg));
}
 ...
 *  dcd::check(a == b, "a does not equal b", 18);
 *  @endcode
 */
inline void check(bool pred, const char* msg, size_t n) {
   if (!pred)
      dcd::detail::assert_or_throw(std::string_view{msg, n});
}
 ...
 *  dcd::check(a == b, msg, 18);
 *  @endcode
 */
inline void check(bool pred, const std::string& msg, size_t n) {
   if (!pred)
      dcd::detail::assert_or_throw(msg.substr(0, n));
}
...
inline void check(bool pred, uint64_t code) {
   if (!pred)
      dcd::detail::assert_or_throw(code);
}
} // namespace dcd
