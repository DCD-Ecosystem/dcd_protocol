...
class symbol_code {
 public:
   ...
   constexpr explicit symbol_code(std::string_view str) : value(0) {
      if (str.size() > 7) {
         dcd::check(false, "string is too long to be a valid symbol_code");
      }
      for (auto itr = str.rbegin(); itr != str.rend(); ++itr) {
         if (*itr < 'A' || *itr > 'Z') {
            dcd::check(false, "only uppercase letters allowed in symbol_code string");
         }
         value <<= 8;
         value |= *itr;
      }
...
DCD_REFLECT(symbol_code, value);
DCD_COMPARE(symbol_code);
...
void from_json(symbol& obj, S& stream) {
   ...
   check(string_to_symbol(obj.value, s.data(), s.data() + s.size()),
      convert_json_error(dcd::from_json_error::expected_symbol));
}
...
DCD_REFLECT(extended_symbol, sym, contract);
DCD_COMPARE(extended_symbol);
} // namespace dcd
