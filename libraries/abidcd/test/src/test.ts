...
const abiAbi = require('../../external/dcdjs/src/abi.abi.json');
const transactionAbi = require('../../external/dcdjs/src/transaction.abi.json');
import * as dcdjs from '../../external/dcdjs/src/dcdjs-api';
import * as dcdjs_jsonrpc from '../../external/dcdjs/src/dcdjs-jsonrpc';
import * as dcdjs_jssig from '../../external/dcdjs/src/dcdjs-jssig';
import * as dcdjs_ser from '../../external/dcdjs/src/dcdjs-serialize';
...
const lib = new fastcall.Library('../build/libabidcd.so')
    .function('void* abidcd_create()')
    .function('void abidcd_destroy(void* context)')
    .function('char* abidcd_get_error(void* context)')
    .function('int abidcd_get_bin_size(void* context)')
    .function('char* abidcd_get_bin_data(void* context)')
    .function('char* abidcd_get_bin_hex(void* context)')
    .function('uint64 abidcd_string_to_name(void* context, char* str)')
    .function('char* abidcd_name_to_string(void* context, uint64 name)')
    .function('int abidcd_set_abi(void* context, uint64 contract, char* abi)')
    .function('int abidcd_set_abi_hex(void* context, uint64 contract, char* hex)')
    .function('char* abidcd_get_type_for_action(void* context, uint64 contract, uint64 action)')
    .function('int abidcd_json_to_bin(void* context, uint64 contract, char* name, char* json)')
    .function('char* abidcd_hex_to_json(void* context, uint64 contract, char* type, char* hex)');

const l = lib.interface;
const cstr = fastcall.makeStringBuffer;

const context = l.abidcd_create();

function check(result: any) {
    if (!result)
        throw new Error(l.abidcd_get_error(context).readCString());
}
...
function name(s: string) {
    return l.abidcd_string_to_name(context, cstr(s));
}

const rpc = new dcdjs_jsonrpc.JsonRpc(rpcEndpoint, { fetch });
const signatureProvider = new dcdjs_jssig.JsSignatureProvider(['5JtUScZK2XEp3g9gh7F8bwtPTRAkASmNrrftmx4AxDKD5K4zDnr']);
const textEncoder = new (require('util').TextEncoder);
const textDecoder = new (require('util').TextDecoder)('utf-8', { fatal: true });
const api = new dcdjs.Api({ rpc, signatureProvider, chainId: null, textEncoder, textDecoder });
const abiTypes = dcdjs_ser.getTypesFromAbi(dcdjs_ser.createInitialTypes(), abiAbi);
const js2Types = dcdjs_ser.getTypesFromAbi(dcdjs_ser.createInitialTypes(), transactionAbi);

function dcdjs_hex_abi_to_json(hex: string): any {
    return api.rawAbiToJson(dcdjs_ser.hexToUint8Array(hex));
}

function dcdjs_json_abi_to_hex(abi: any) {
    let buf = new dcdjs_ser.SerialBuffer({ textEncoder, textDecoder });
    ...
    return dcdjs_ser.arrayToHex(buf.asUint8Array());
}

function abidcd_json_to_hex(contract: number, type: string, data: string) {
    check(l.abidcd_json_to_bin(context, contract, cstr(type), cstr(data)));
    return l.abidcd_get_bin_hex(context).readCString();
}

function abidcd_hex_to_json(contract: number, type: string, hex: string) {
    let result = l.abidcd_hex_to_json(context, contract, cstr(type), cstr(hex));
    checkPtr(result);
    return result.readCString();
}

function dcdjs_json_to_hex(types: any, type: string, data: any) {
    let js2Type = dcdjs_ser.getType(types, type);
    let buf = new dcdjs_ser.SerialBuffer({ textEncoder, textDecoder });
    js2Type.serialize(buf, data);
    return dcdjs_ser.arrayToHex(buf.asUint8Array());
}

function dcdjs_hex_to_json(types: any, type: string, hex: string) {
    let js2Type = dcdjs_ser.getType(types, type);
    let buf = new dcdjs_ser.SerialBuffer({ textEncoder, textDecoder, array: dcdjs_ser.hexToUint8Array(hex) });
...
function check_type(contract: number, types: any, type: string, data: string, expected = data) {
    let hex = abidcd_json_to_hex(contract, type, data);
    let json = abidcd_hex_to_json(contract, type, hex);
    ...
    let js2Hex = dcdjs_ser.arrayToHex(buf.asUint8Array()).toUpperCase();
    //console.log(hex)
    //console.log(js2Hex)
    if (js2Hex != hex)
        throw new Error('dcdjs hex mismatch');
    let js2Json = JSON.stringify(js2Type.deserialize(buf));
    //console.log(json);
    //console.log(js2Json);
    if (js2Json != json)
        throw new Error('dcdjs json mismatch');
}

function check_types() {
    let token = name('dcd.token');
    let test = name('test.abi');
    check(l.abidcd_set_abi_hex(context, token, cstr(tokenHexApi)));
    check(l.abidcd_set_abi(context, test, cstr(testAbi)));
    const tokenTypes = dcdjs_ser.getTypesFromAbi(dcdjs_ser.createInitialTypes(), dcdjs_hex_abi_to_json(tokenHexApi));
    const testTypes = dcdjs_ser.getTypesFromAbi(dcdjs_ser.createInitialTypes(), dcdjs_hex_abi_to_json(dcdjs_json_abi_to_hex(JSON.parse(testAbi))));

    check_throw('Error: missing abi_def.version (type=string)', () => dcdjs_hex_abi_to_json(dcdjs_json_abi_to_hex({})));
    check_throw('Error: Unsupported abi version', () => dcdjs_hex_abi_to_json(dcdjs_json_abi_to_hex({ version: '' })));
    check_throw('Error: Unsupported abi version', () => dcdjs_hex_abi_to_json(dcdjs_json_abi_to_hex({ version: 'dcd::abi/9.0' })));
    dcdjs_hex_abi_to_json(dcdjs_json_abi_to_hex({ version: 'dcd::abi/1.0' }));
    dcdjs_hex_abi_to_json(dcdjs_json_abi_to_hex({ version: 'dcd::abi/1.1' }));

    check_type(0, js2Types, 'bool', 'true');
    check_type(0, js2Types, 'bool', 'false');
    check_throw('Error: Read past end of buffer', () => dcdjs_hex_to_json(js2Types, 'bool', ''));
    check_throw('Error: Expected true or false', () => dcdjs_json_to_hex(js2Types, 'bool', 'trues'));
    check_throw('Error: Expected true or false', () => dcdjs_json_to_hex(js2Types, 'bool', null));
    ...
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'int8', 128));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'int8', -129));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint8', -1));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint8', 256));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'int8', '128'));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'int8', '-129'));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint8', '-1'));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint8', '256'));
    ...
    check_throw('Error: Read past end of buffer', () => dcdjs_hex_to_json(js2Types, 'int16', '01'));
    check_type(0, js2Types, 'uint16', '0');
    check_type(0, js2Types, 'uint16', '65535');
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'int16', 32768));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'int16', -32769));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint16', -1));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint16', 655356));
    ...
    check_throw('Error: Expected number', () => dcdjs_json_to_hex(js2Types, 'int32', 'foo'));
    check_throw('Error: Expected number', () => dcdjs_json_to_hex(js2Types, 'int32', true));
    check_throw('Error: Expected number', () => dcdjs_json_to_hex(js2Types, 'int32', []));
    check_throw('Error: Expected number', () => dcdjs_json_to_hex(js2Types, 'int32', {}));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'int32', '2147483648'));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'int32', '-2147483649'));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint32', '-1'));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint32', '4294967296'));
    ...
    check_throw('Error: number is out of range', () => dcdjs_json_to_hex(js2Types, 'int64', '9223372036854775808'));
    check_throw('Error: number is out of range', () => dcdjs_json_to_hex(js2Types, 'int64', '-9223372036854775809'));
    check_throw('Error: invalid number', () => dcdjs_json_to_hex(js2Types, 'uint64', '-1'));
    check_throw('Error: number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint64', '18446744073709551616'));
    ...
    check_throw('Error: number is out of range', () => dcdjs_json_to_hex(js2Types, 'int128', '170141183460469231731687303715884105728'));
    check_throw('Error: number is out of range', () => dcdjs_json_to_hex(js2Types, 'int128', '-170141183460469231731687303715884105729'));
    check_throw('Error: invalid number', () => dcdjs_json_to_hex(js2Types, 'int128', 'true'));
    check_throw('Error: invalid number', () => dcdjs_json_to_hex(js2Types, 'uint128', '-1'));
    check_throw('Error: number is out of range', () => dcdjs_json_to_hex(js2Types, 'uint128', '340282366920938463463374607431768211456'));
    check_throw('Error: invalid number', () => dcdjs_json_to_hex(js2Types, 'uint128', 'true'));
    ...
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'varint32', '2147483648'));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'varint32', '-2147483649'));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'varuint32', '-1'));
    check_throw('Error: Number is out of range', () => dcdjs_json_to_hex(js2Types, 'varuint32', '4294967296'));
    ...
    check_throw('Error: Invalid time format', () => dcdjs_json_to_hex(js2Types, 'time_point_sec', true));
    ...
    check_throw('Error: Invalid time format', () => dcdjs_json_to_hex(js2Types, 'time_point', true));
    ...
    check_throw('Error: Invalid time format', () => dcdjs_json_to_hex(js2Types, 'block_timestamp_type', true));
    ...
    check_throw('Error: Expected string containing name', () => dcdjs_json_to_hex(js2Types, 'name', true));
    check_type(0, js2Types, 'bytes', '""');
    check_type(0, js2Types, 'bytes', '"00"');
    check_type(0, js2Types, 'bytes', '"AABBCCDDEEFF00010203040506070809"');
    check_throw('Error: Odd number of hex digits', () => dcdjs_json_to_hex(js2Types, 'bytes', '"0"'));
    check_throw('Error: Expected hex string', () => dcdjs_json_to_hex(js2Types, 'bytes', '"yz"'));
    check_throw('Error: Expected string containing hex digits', () => dcdjs_json_to_hex(js2Types, 'bytes', true));
    check_throw('Error: Read past end of buffer', () => dcdjs_hex_to_json(js2Types, 'bytes', "01"));
    ...
    check_throw('Error: Read past end of buffer', () => dcdjs_hex_to_json(js2Types, 'string', '01'));
    ...
    check_throw('Error: Expected hex string', () => dcdjs_json_to_hex(js2Types, 'checksum256', 'yz'));
    check_throw('Error: Expected string containing hex digits', () => dcdjs_json_to_hex(js2Types, 'checksum256', true));
    check_throw('Error: Binary data has incorrect size', () => dcdjs_json_to_hex(js2Types, 'checksum256', 'a0'));
    check_type(0, js2Types, 'public_key', '"DCD1111111111111111111111111111111114T1Anm"', '"PUB_K1_11111111111111111111111111111111149Mr2R"');
    check_type(0, js2Types, 'public_key', '"DCD11111111111111111111111115qCHTcgbQwptSz99m"', '"PUB_K1_11111111111111111111111115qCHTcgbQwpvP72Uq"');
    check_type(0, js2Types, 'public_key', '"DCD111111111111111114ZrjxJnU1LA5xSyrWMNuXTrYSJ57"', '"PUB_K1_111111111111111114ZrjxJnU1LA5xSyrWMNuXTrVub2r"');
    check_type(0, js2Types, 'public_key', '"DCD1111111113diW7pnisfdBvHTXP7wvW5k5Ky1e5DVuF23dosU"', '"PUB_K1_1111111113diW7pnisfdBvHTXP7wvW5k5Ky1e5DVuF4PizpM"');
    check_type(0, js2Types, 'public_key', '"DCD11DsZ6Lyr1aXpm9aBqqgV4iFJpNbSw5eE9LLTwNAxqjJgmjgbT"', '"PUB_K1_11DsZ6Lyr1aXpm9aBqqgV4iFJpNbSw5eE9LLTwNAxqjJgXSdB8"');
    check_type(0, js2Types, 'public_key', '"DCD12wkBET2rRgE8pahuaczxKbmv7ciehqsne57F9gtzf1PVYNMRa2"', '"PUB_K1_12wkBET2rRgE8pahuaczxKbmv7ciehqsne57F9gtzf1PVb7Rf7o"');
    check_type(0, js2Types, 'public_key', '"DCD1yp8ebBuKZ13orqUrZsGsP49e6K3ThVK1nLutxSyU5j9SaXz9a"', '"PUB_K1_1yp8ebBuKZ13orqUrZsGsP49e6K3ThVK1nLutxSyU5j9Tx1r96"');
    check_type(0, js2Types, 'public_key', '"DCD9adaAMuB9v8yX1mZ5PtoB6VFSCeqRGjASd8ZTM6VUkiHL7mue4K"', '"PUB_K1_9adaAMuB9v8yX1mZ5PtoB6VFSCeqRGjASd8ZTM6VUkiHLB5XEdw"');
    check_type(0, js2Types, 'public_key', '"DCD69X3383RzBZj41k73CSjUNXM5MYGpnDxyPnWUKPEtYQmTBWz4D"', '"PUB_K1_69X3383RzBZj41k73CSjUNXM5MYGpnDxyPnWUKPEtYQmVzqTY7"');
    check_type(0, js2Types, 'public_key', '"DCD7yBtksm8Kkg85r4in4uCbfN77uRwe82apM8jjbhFVDgEgz3w8S"', '"PUB_K1_7yBtksm8Kkg85r4in4uCbfN77uRwe82apM8jjbhFVDgEcarGb8"');
    check_type(0, js2Types, 'public_key', '"DCD7WnhaKwHpbSidYuh2DF1qAExTRUtPEdZCaZqt75cKcixuQUtdA"', '"PUB_K1_7WnhaKwHpbSidYuh2DF1qAExTRUtPEdZCaZqt75cKcixtU7gEn"');
    check_type(0, js2Types, 'public_key', '"DCD7Bn1YDeZ18w2N9DU4KAJxZDt6hk3L7eUwFRAc1hb5bp6xJwxNV"', '"PUB_K1_7Bn1YDeZ18w2N9DU4KAJxZDt6hk3L7eUwFRAc1hb5bp6uEBZA8"');
    ...
    check_throw('Error: expected string containing public key', () => dcdjs_json_to_hex(js2Types, 'public_key', true));
    check_throw('Error: unrecognized public key format', () => dcdjs_json_to_hex(js2Types, 'public_key', 'foo'));
    ...
    check_throw('Error: expected string containing private key', () => dcdjs_json_to_hex(js2Types, 'private_key', true));
    check_throw('Error: unrecognized private key type', () => dcdjs_json_to_hex(js2Types, 'private_key', 'foo'));
    ...
    check_throw('Error: expected string containing signature', () => dcdjs_json_to_hex(js2Types, 'signature', true));
...
async function push_transfer() {
    if (useTokenHexApi)
        check(l.abidcd_set_abi_hex(context, name('dcd.token'), cstr(tokenHexApi)));
    else
        check(l.abidcd_set_abi(context, name('dcd.token'), jsonStr((await rpc.get_abi('dcd.token')).abi)));
    let type = checkPtr(l.abidcd_get_type_for_action(context, name('dcd.token'), name('transfer')));
    ...
    console.log('action bin->json: ', abidcd_hex_to_json(name('dcd.token'), 'transfer', actionDataHex));

    let info = await rpc.get_info();
    let refBlock = await rpc.get_block(info.head_block_num - 3);
    let transaction = {
        expiration: dcdjs_ser.timePointSecToDate(dcdjs_ser.dateToTimePointSec(refBlock.timestamp) + 10),
    ...
    check(l.abidcd_json_to_bin(context, 0, cstr('transaction'), jsonStr(transaction)));
    let transactionDataHex = l.abidcd_get_bin_hex(context).readCString();
    console.log('transaction json->bin: ', transactionDataHex);
    console.log('transaction bin->json: ', abidcd_hex_to_json(0, 'transaction', transactionDataHex));

    let sig = await signatureProvider.sign({
        chainId: info.chain_id,
        requiredKeys: await signatureProvider.getAvailableKeys(),
        serializedTransaction: dcdjs_ser.hexToUint8Array(transactionDataHex),
        abis: [],
    });
    console.log('sig:', sig)
...
(async () => {
    try {
        check(context);
        check(l.abidcd_set_abi(context, 0, jsonStr(transactionAbi)));
        check_types();
        if (useRpcEndpoint)
            await push_transfer();
    } catch (e) {
