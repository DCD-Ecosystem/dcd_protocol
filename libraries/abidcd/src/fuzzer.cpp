...
extern "C" int LLVMFuzzerTestOneInput(const char* pos, size_t size) {
    ...
    auto context = abidcd_create();
    bool ok;
    if (header.abi_is_bin)
        ok = abidcd_set_abi_bin(context, header.contract, abi, header.abi_size);
    else
        ok = abidcd_set_abi(context, header.contract, abi);

    if (ok) {
        if (header.operation == fuzzer_json_to_bin) {
            abidcd_json_to_bin(context, header.contract, type, data);
            // printf("%s\n", abidcd_get_bin_hex(context));
            abidcd_json_to_bin_reorderable(context, header.contract, type, data);
            // printf("%s\n", abidcd_get_bin_hex(context));
        } else {
            abidcd_bin_to_json(context, header.contract, type, data, end - data);
        }
    }
