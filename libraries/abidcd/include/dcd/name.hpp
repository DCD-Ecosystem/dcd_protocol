...
struct name {
   ...
   explicit    operator std::string() const { return dcd::name_to_string(value); }
   ...
   static constexpr uint8_t char_to_value(char c) {
      if (c == '.')
         ...
         dcd::check(false, "character is not in allowed character set for names");
...
void to_json(const name& obj, S& stream) {
   to_json(dcd::name_to_string(obj.value), stream);
}
