...
struct asset {
   ...
   asset(int64_t a, class symbol s) : amount(a), symbol{ s } {
      dcd::check(is_amount_within_range(), "magnitude of asset amount must be less than 2^62");
      dcd::check(symbol.is_valid(), "invalid symbol name");
   }
   ...
   asset& operator-=(const asset& a) {
      dcd::check(a.symbol == symbol, "attempt to subtract asset with different symbol");
      amount -= a.amount;
      dcd::check(-max_amount <= amount, "subtraction underflow");
      dcd::check(amount <= max_amount, "subtraction overflow");
      return *this;
   }
    ...
    */
   asset& operator+=(const asset& a) {
      dcd::check(a.symbol == symbol, "attempt to add asset with different symbol");
      amount += a.amount;
      dcd::check(-max_amount <= amount, "addition underflow");
      dcd::check(amount <= max_amount, "addition overflow");
      return *this;
   }
   ...
   asset& operator*=(int64_t a) {
      ...
      dcd::check(tmp <= max_amount, "multiplication overflow");
      dcd::check(tmp >= -max_amount, "multiplication underflow");
      amount = (int64_t)tmp;
      return *this;
   }
#endif
    ...
    */
   asset& operator/=(int64_t a) {
      dcd::check(a != 0, "divide by zero");
      dcd::check(!(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow");
      amount /= a;
      return *this;
   }
   ...
   friend int64_t operator/(const asset& a, const asset& b) {
      dcd::check(b.amount != 0, "divide by zero");
      dcd::check(a.symbol == b.symbol, "comparison of assets with different symbols is not allowed");
...
DCD_REFLECT(asset, amount, symbol);

template <typename S>
inline void from_string(asset& result, S& stream) {
   int64_t  amount;
   uint64_t sym;
   check(dcd::string_to_asset(amount, sym, stream.pos, stream.end, true),
      convert_stream_error(dcd::stream_error::invalid_asset_format));
...
void from_json(asset& obj, S& stream) {
   ...
   check(string_to_asset(obj.amount, obj.symbol.value, s.data(), s.data() + s.size()),
      convert_json_error(dcd::from_json_error::expected_symbol_code));
}
...
struct extended_asset {
   ...
   friend extended_asset operator-(const extended_asset& a, const extended_asset& b) {
      dcd::check(a.contract == b.contract, "type mismatch");
...
DCD_REFLECT(extended_asset, quantity, contract);
} // namespace dcd
