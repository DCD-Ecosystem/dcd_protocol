...
constexpr inline std::string_view convert_abi_error(dcd::abi_error e) {
   switch (e) {
...
DCD_REFLECT(type_def, new_type_name, type);
...
DCD_REFLECT(field_def, name, type);
...
DCD_REFLECT(struct_def, name, base, fields);
...
DCD_REFLECT(action_def, name, type, ricardian_contract);
...
DCD_REFLECT(table_def, name, index_type, key_names, key_types, type);
...
DCD_REFLECT(clause_pair, id, body);
...
DCD_REFLECT(error_message, error_code, error_msg);
...
DCD_REFLECT(variant_def, name, types);
...
DCD_REFLECT(action_result_def, name, result_type);
...
DCD_REFLECT(primary_key_index_def, name, type);

struct secondary_index_def {
   std::string type;
};

DCD_REFLECT(secondary_index_def, type);

struct kv_table_entry_def {
   std::string                                type;
   primary_key_index_def                      primary_index;
   std::map<dcd::name, secondary_index_def> secondary_indices;
};

DCD_REFLECT(kv_table_entry_def, type, primary_index, secondary_indices);
...
struct abi_def {
   ...
   might_not_exist<std::map<dcd::name, kv_table_entry_def>> kv_tables{};
};

DCD_REFLECT(abi_def, version, types, structs, actions, tables, ricardian_clauses, error_messages, abi_extensions,
...
struct abi {
   std::map<dcd::name, std::string> action_types;
   std::map<dcd::name, std::string> table_types;
   std::map<dcd::name, std::string> kv_tables;
   std::map<std::string, abi_type>    abi_types;
   std::map<dcd::name, std::string> action_result_types;
