...
void from_json(fixed_bytes<Size, T>& obj, S& stream) {
   std::vector<char> v;
   dcd::from_json_hex(v, stream);
   check(v.size() == Size, convert_json_error(dcd::from_json_error::hex_string_incorrect_length));
...
void to_json(const fixed_bytes<Size, T>& obj, S& stream) {
   ...
   dcd::to_json_hex((const char*)bytes.data(), bytes.size(), stream);
}
