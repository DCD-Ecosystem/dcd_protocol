...
protected:
    ...
    void TestEncodedOutputStream(const char* expectedFilename, bool putBOM) {
        // Test FileWriteStream
        {
            ...
            EncodedOutputStream<FileEncoding, FileWriteStream> dcd(os, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, dcd);
                EXPECT_TRUE(success);
            }
            dcd.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            EncodedOutputStream<FileEncoding, MemoryBuffer> dcd(mb, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, dcd);
                EXPECT_TRUE(success);
    ...
    void TestAutoUTFOutputStream(UTFType type, bool putBOM, const char *expectedFilename) {
        // Test FileWriteStream
        {
            ...
            char buffer[16];
            FileWriteStream os(fp, buffer, sizeof(buffer));
            AutoUTFOutputStream<unsigned, FileWriteStream> dcd(os, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, dcd);
                EXPECT_TRUE(success);
            }
            dcd.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            AutoUTFOutputStream<unsigned, MemoryBuffer> dcd(mb, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, dcd);
                EXPECT_TRUE(success);
