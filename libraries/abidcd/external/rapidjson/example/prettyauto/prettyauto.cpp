...
int main(int, char*[]) {
    ...
    OutputStream dcd(os, eis.GetType(), eis.HasBOM());                      // NEW
    PrettyWriter<OutputStream, UTF8<>, AutoUTF<unsigned> > writer(dcd);     // CHANGED
#else
    ...
    OutputStream dcd(os, true);                                             // NEW
    PrettyWriter<OutputStream, UTF8<>, UTF16LE<> > writer(dcd);             // CHANGED
#endif
