...
#include "dcd_test_generator.hpp"
...
const string include_dcd = "#include <dcd/dcd.hpp>\n\n";
...
map<string, vector<picojson::object>> get_file_func_mappings(picojson::value v) {
   ...
   picojson::array commands = o["commands"].get<picojson::array>();
...
int main(int argc, char** argv) {
   ...
   if (argc != 2 || !strcmp(argv[1], "-h") || !strcmp(argv[1], "--help")) {
      usage(argc ? argv[0] : "dcd_test_generator");
   }
   ifs.open(argv[1]);
   if (!ifs) {
