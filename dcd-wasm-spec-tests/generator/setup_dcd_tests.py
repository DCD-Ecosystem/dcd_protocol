...
from compile_dcd_tests import CompileError

import compile_dcd_tests
import generate_dcd_tests
...
def generate_wasm_and_copy():
    ...
    for d in os.listdir():
        ...
        if wasm_file in altered_wasms:
            ...
            try:
                generate_dcd_tests.main(g_wasm_file, t_wasm_file, o_wast_file, map_file)
                wasm_file_path = os.path.join(d, wasm_file)
                out = subprocess.run(
                    ['dcd-wast2wasm', o_wast_file, '-o', os.path.join(d, wasm_file)],
                    capture_output=True
                )
...
if __name__ == '__main__':
    ...
    generator = os.path.join(GENERATOR_DIR, 'dcd_test_generator')
