...
using watermark_t = std::pair<uint32_t, dcd::chain::block_timestamp_type>;

class sync_callback {
 public:
   virtual void on_snapshot(const char* snapshot_filename)     = 0;
   virtual void on_block(dcd::chain::signed_block_ptr block) = 0;
};
...
class block_vault_interface {
 public:
   ...
   /// will guarantee that all future dcdnode nodes will know about this block OR
   ...
   ///
   /// \brief The primary method for a dcdnode node to offer snapshot data to
   ...
   ///
   /// \brief The primary method for bringing a new dcdnode node into sync with
   /// the Block Vault. This is the primary method for bringing a new dcdnode
   /// node into sync with the Block Vault. Syncing is semi-session based, a
   /// syncing dcdnode will establish a session with a single Block Vault node,
   ...
   ///
   virtual void sync(const dcd::chain::block_id_type* block_id, sync_callback& callback) = 0;
};
