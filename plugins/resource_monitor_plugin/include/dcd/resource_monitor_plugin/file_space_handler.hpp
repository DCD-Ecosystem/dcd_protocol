...
namespace dcd::resource_monitor {
         ...
         for (auto& fs: filesystems) {
            ...
            if ( info.available < fs.shutdown_available ) {
               ...
               if ( shutdown_on_exceeded) {
                  wlog("dcdnode will shutdown when space usage exceeds threshold ${threshold}%", ("threshold", shutdown_threshold));
               }
            }
         }
   ...
   void space_monitor_loop() {
      if ( is_threshold_exceeded() && shutdown_on_exceeded ) {
         wlog("Shutting down");
         appbase::app().quit(); // This will gracefully stop dcdnode
         return;
      }
