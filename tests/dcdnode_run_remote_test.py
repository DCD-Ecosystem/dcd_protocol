...
###############################################################
# dcdnode_run_remote_test
#
#  Tests remote capability of the dcdnode_run_test. Test will setup cluster and pass nodes info to dcdnode_run_test. E.g.
#  dcdnode_run_remote_test.py -v --clean-run --dump-error-detail
#
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/dcdnode_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("dcdnode", actualTest))
    Print("cmd: %s\n" % (cmd))
