...
###############################################################
# dcdnode_chainbase_allocation_test
#
...
try:
    ...
    # (automatically taken care by the automatically generated dcd account)
    assert cluster.launch(
        ...
        specificExtradcdnodeArgs={
            1:"--read-mode irreversible --plugin dcd::producer_api_plugin"})
    ...
    setProdsStr += ']}'
    cmd="push action -j dcd setprods '{}' -p dcd".format(setProdsStr)
    trans = producerNode.processdcdcliCmd(cmd, cmd, silentErrors=False)
    assert trans
