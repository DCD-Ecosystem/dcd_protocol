...
try:
    ...
    accountsToCreate = [cluster.dcdAccount]
    ...
    Print("Create new accounts via %s" % (cluster.dcdAccount.name))
    # create accounts via dcd as otherwise a bid is needed
    for account in accounts:
        trans = node.createInitializeAccount(account, cluster.dcdAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
    ...
    Print("Transfer funds to new accounts via %s" % (cluster.dcdAccount.name))
    for account in accounts:
        ...
        trans = node.transferFunds(cluster.dcdAccount, account, transferAmount, "test transfer", waitForTransBlock=False, reportStatus=False, sign = True)
        checkTransIds.append(Node.getTransId(trans))
