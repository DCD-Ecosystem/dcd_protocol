...
###############################################################
# dcdnode_under_min_avail_ram
#
# Sets up 4 producing nodes using --chain-state-db-guard-size-mb and --chain-state-db-size-mb to verify that dcdnode will
...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.dcdAccount])
    ...
    for account in accounts:
        ...
        trans=nodes[0].createInitializeAccount(account, cluster.dcdAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        ...
        nodes[0].transferFunds(cluster.dcdAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.dcdAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.dcdAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.dcdAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.dcdAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                        Print("Failed to push create action to dcd contract for %d consecutive times, looks like dcdnode already exited." % (timeOutCount))
                        keepProcessing=False
                        break

                    Print("Failed to push create action to dcd contract. sleep for 5 seconds")
    ...
    # min and max are subjective, just assigned to make sure that many small changes in dcdnode don't 
    ...
    while keepProcessing and count < 40:
        ...
        for fromIndexOffset in range(namedAccounts.numAccounts):
            ...
            try:
                trans=nodes[count % numNodes].pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Print("Failed to push create action to dcd contract. sleep for 60 seconds")
                    time.sleep(60)
                time.sleep(1)
            except TypeError as ex:
