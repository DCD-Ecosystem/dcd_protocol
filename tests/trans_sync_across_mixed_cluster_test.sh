...
getHeadBlockNum()
{
  INFO="$(programs/dcdcli/dcdcli get info)"
  verifyErrorCode "dcdcli get info"
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/dcdcli/dcdcli --port $port2 get block 1)"
    verifyErrorCode "dcdcli get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/dcdcli/dcdcli create key)"
verifyErrorCode "dcdcli create key"
...
PASSWORD_INITA="$(programs/dcdcli/dcdcli wallet create --name inita)"
verifyErrorCode "dcdcli wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/dcdcli/dcdcli wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "dcdcli wallet import"
programs/dcdcli/dcdcli wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "dcdcli wallet import"
programs/dcdcli/dcdcli wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "dcdcli wallet import"
programs/dcdcli/dcdcli wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "dcdcli wallet import"
...
ACCOUNT_INFO="$(programs/dcdcli/dcdcli create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "dcdcli create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/dcdcli/dcdcli get account testera)"
verifyErrorCode "dcdcli get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/dcdcli/dcdcli transfer inita testera 975321 "test transfer")"
    verifyErrorCode "dcdcli transfer"
   ...
   TRANS_INFO="$(programs/dcdcli/dcdcli --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "dcdcli get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
