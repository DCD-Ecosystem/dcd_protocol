...
try:
    ...
    for nodeNum in range(txnGenNodeNum, txnGenNodeNum+startedNonProdNodes):
        specificExtradcdnodeArgs[nodeNum]="--plugin dcd::txn_test_gen_plugin --txn-test-gen-account-prefix txntestacct"
    ...
    if cluster.launch(prodCount=prodCount, onlyBios=False, pnodes=pnodes, totalNodes=totalNodes, totalProducers=pnodes*prodCount,
                      useBiosBootFile=False, specificExtradcdnodeArgs=specificExtradcdnodeArgs, unstartedNodes=catchupCount, loadSystemContract=False) is False:
    ...
    txnGenNodes[0].txnGenCreateTestAccounts(cluster.dcdAccount.name, cluster.dcdAccount.activePrivateKey)
