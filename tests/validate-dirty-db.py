...
# validate-dirty-db
#
# Test for validating the dirty db flag sticks repeated dcdnode restart attempts
#
###############################################################
...
def rundcdnodeAndGetOutput(myTimeout=3):
    """Startup dcdnode, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching dcdnode process.")
    cmd="programs/dcdnode/dcdnode --config-dir etc/dcd/node_bios --data-dir var/lib/node_bios --verbose-http-errors --http-validate-host=false"
    Print("cmd: %s" % (cmd))
    proc=subprocess.Popen(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if debug: Print("dcdnode process launched.")

    output={}
    try:
        if debug: Print("Setting dcdnode process timeout.")
        outs,errs = proc.communicate(timeout=myTimeout)
        if debug: Print("dcdnode process has exited.")
        output["stdout"] = outs.decode("utf-8")
        output["stderr"] = errs.decode("utf-8")
        output["returncode"] = proc.returncode
    except (subprocess.TimeoutExpired) as _:
        Print("ERROR: dcdnode is running beyond the defined wait time. Hard killing dcdnode instance.")
...
try:
    ...
    Print("Restart dcdnode repeatedly to ensure dirty database flag sticks.")
    timeout=6

    for i in range(1,4):
        Print("Attempt %d." % (i))
        ret = rundcdnodeAndGetOutput(timeout)
        assert(ret)
        assert(isinstance(ret, tuple))
        if not ret[0]:
            errorExit("Failed to startup dcdnode successfully on try number %d" % (i))
        assert(ret[1])
