...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="dcd-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is dcdksd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899
        ...
        self.dcdAccount=self.defProducerAccounts["dcd"]= Account("dcd")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extradcdnodeArgs="", useBiosBootFile=True, specificExtradcdnodeArgs=None, onlySetProds=False,
        ...
        extradcdnodeArgs: string of arguments to pass through to each nodoes instance (via --dcdnode flag on launcher)
        ...
        specificExtradcdnodeArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-dcdnode flags on launcher), example: { "5" : "--plugin dcd::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the dcd.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on dcdnode default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        dcdnodeArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            dcdnodeArgs += " --plugin dcd::wallet_api_plugin"
        if Utils.Debug:
            dcdnodeArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            dcdnodeArgs += " --plugin dcd::producer_api_plugin"

        if len(manualProducerNodeConf) > 0 and specificExtradcdnodeArgs is None:
            specificExtradcdnodeArgs = {}

        for node, conf in manualProducerNodeConf.items():
            arg = specificExtradcdnodeArgs.get(node, "")
            account = conf['key']
            arg = arg + " --plugin dcd::producer_plugin --signature-provider {}=KEY:{} ".format(account.ownerPublicKey, account.ownerPrivateKey)
            for name in conf['names']:
                arg = arg + "--producer-name {} ".format(name)
            specificExtradcdnodeArgs[node] = arg

        Utils.Print("specificExtradcdnodeArgs=", specificExtradcdnodeArgs)

        httpMaxResponseTimeSet = False
        if specificExtradcdnodeArgs is not None:
            assert(isinstance(specificExtradcdnodeArgs, dict))
            for nodeNum,arg in specificExtradcdnodeArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-dcdnode")
                if arg.find("--http-max-response-time-ms") != -1:
                    httpMaxResponseTimeSet = True
                cmdArr.append(arg)

        if not httpMaxResponseTimeSet and extradcdnodeArgs.find("--http-max-response-time-ms") == -1:
            extradcdnodeArgs+=" --http-max-response-time-ms 990000 "

        if extradcdnodeArgs is not None:
            assert(isinstance(extradcdnodeArgs, str))
            dcdnodeArgs += extradcdnodeArgs

        if dcdnodeArgs:
            cmdArr.append("--dcdnode")
            cmdArr.append(dcdnodeArgs)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-dcdnode")
                cmdArr.append(path)
        ...
        self.dcdAccount=self.defProducerAccounts["dcd"]
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        receiving transferAmount*n DCD and forwarding x-transferAmount funds. Transfer actions are spread round-robin across the cluster to vaidate system cohesiveness."""
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.dcdAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse dcd private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by dcd-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/dcd.bios",
            "BIOS_CURRENCY_SYMBOL": CORE_SYMBOL,
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/dcd.bios"
        ...
        dcdName="dcd"
        dcdKeys=producerKeys[dcdName]
        dcdAccount=Account(dcdName)
        dcdAccount.ownerPrivateKey=dcdKeys["private"]
        dcdAccount.ownerPublicKey=dcdKeys["public"]
        dcdAccount.activePrivateKey=dcdKeys["private"]
        dcdAccount.activePublicKey=dcdKeys["public"]
        producerKeys.pop(dcdName)

        if not self.walletMgr.importKey(dcdAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (dcdName))
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"dcd\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init dcd transfer")
            opts="--permission dcd@active"
            if name != "dcd":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from dcd.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True, manualProducerNodeConf=[]):
        """Create 'prodCount' init accounts and deposits 10000000000 DCD in each. If prodCount is -1 will initialize all possible producers.
        ...
        trans=biosNode.publishContract(dcdAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, dcdAccount, 0)
            if trans is None:
        ...
        for node, conf in manualProducerNodeConf.items():
            ...
            for name in conf['names']:
                account.name = name
                trans=biosNode.createAccount(account, dcdAccount, 0)
                if trans is None:
                    Utils.Print("ERROR: Failed to create account %s" % (name))
                    return None
                Node.validateTransaction(trans)
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission dcd@active"
                    myTrans=biosNode.pushMessage("dcd", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("dcd", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but dcd).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "dcd"
        ...
        dcdTokenAccount=copy.deepcopy(dcdAccount)
        dcdTokenAccount.name="dcd.token"
        trans=biosNode.createAccount(dcdTokenAccount, dcdAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dcdTokenAccount.name))
            return None

        dcdRamAccount=copy.deepcopy(dcdAccount)
        dcdRamAccount.name="dcd.ram"
        trans=biosNode.createAccount(dcdRamAccount, dcdAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dcdRamAccount.name))
            return None

        dcdRamfeeAccount=copy.deepcopy(dcdAccount)
        dcdRamfeeAccount.name="dcd.ramfee"
        trans=biosNode.createAccount(dcdRamfeeAccount, dcdAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dcdRamfeeAccount.name))
            return None

        dcdStakeAccount=copy.deepcopy(dcdAccount)
        dcdStakeAccount.name="dcd.stake"
        trans=biosNode.createAccount(dcdStakeAccount, dcdAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dcdStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(dcdTokenAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=dcdTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (dcdAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to dcd contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (dcdAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (dcdAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to dcd contract.")
            return None
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (dcdAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (dcdAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (dcdTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (dcdAccount.name)
            trans=biosNode.pushMessage(dcdAccount.name, action, data, opts)
    ...
    def bounce(self, nodes, silent=True):
        """Bounces dcdnode instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down dcdnode instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
