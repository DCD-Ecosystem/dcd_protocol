...
###############################################################
# dcdnode_multiple_version_protocol_feature_test
#
# Test for verifying that older versions of dcdnode can work with newer versions of dcdnode.
#
###############################################################
...
def restartNode(node: Node, chainArg=None, addSwapFlags=None, dcdnodePath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, cachePopen=True, dcdnodePath=dcdnodePath)
...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extradcdnodeArgs=" --plugin dcd::producer_api_plugin ",
    ...
    # Finally, when we restart the 4th node with the version of dcdnode that supports protocol feature,
    ...
    oldNode.relaunch(chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, dcdnodePath="programs/dcdnode/dcdnode")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, chainArg="--replay", dcdnodePath="programs/dcdnode/dcdnode")
