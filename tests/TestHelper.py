...
class TestHelper(object):
    ...
    def parse_args(includeArgs, applicationSpecificArgs=AppArgs()):
        ...
        if "--kill-count" in includeArgs:
            parser.add_argument("--kill-count", type=int, help="dcdnode instances to kill", default=-1)
        ...
        if "--host" in includeArgs:
            parser.add_argument("-h", "--host", type=str, help="%s host name" % (Utils.DcdServerName),
                                     default=TestHelper.LOCAL_HOST)
        if "--port" in includeArgs:
            parser.add_argument("--port", type=int, help="%s host port" % Utils.DcdServerName,
                                     default=TestHelper.DEFAULT_PORT)
        if "--wallet-host" in includeArgs:
            parser.add_argument("--wallet-host", type=str, help="%s host" % Utils.DcdWalletName,
                                     default=TestHelper.LOCAL_HOST)
        if "--wallet-port" in includeArgs:
            parser.add_argument("--wallet-port", type=int, help="%s port" % Utils.DcdWalletName,
        ...
        if "--clean-run" in includeArgs:
            parser.add_argument("--clean-run", help="Kill all dcdnode and kldcd instances", action='store_true')
        if "--sanity-test" in includeArgs:
            parser.add_argument("--sanity-test", help="Validates dcdnode and kldcd are in path and can be started up.", action='store_true')
    ...
    def printSystemInfo(prefix):
        ...
        Utils.Print("DCD Client version: %s" % (clientVersion))
        Utils.Print("Processor: %s" % (platform.processor()))
        Utils.Print("OS name: %s" % (platform.platform()))
    
    @staticmethod
    # pylint: disable=too-many-arguments
    def shutdown(cluster, walletMgr, testSuccessful=True, killDcdInstances=True, killWallet=True, keepLogs=False, cleanRun=True, dumpErrorDetails=False):
        ...
        assert(isinstance(killDcdInstances, bool))
        ...
        if not testSuccessful and dumpErrorDetails:
            ...
            Utils.Print(Utils.FileDivider)
            psOut = Cluster.pgrepDcdServers(timeout=60)
