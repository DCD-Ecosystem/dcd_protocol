...
The `http_plugin` is a core plugin supported by both `dcdnode` and `dcdksd`. The plugin is required to enable any RPC API functionality provided by a `dcdnode` or `dcdksd` instance.
...
```sh
# command-line
dcdnode ... --plugin dcd::http_plugin [options]
 (or)
dcdksd ... --plugin dcd::http_plugin [options]
```
