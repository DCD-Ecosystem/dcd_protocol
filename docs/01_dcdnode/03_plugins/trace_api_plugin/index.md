...
```sh
# command-line
dcdnode ... --plugin dcd::trace_api_plugin [options]
```
...
```console
Config Options for dcd::trace_api_plugin:
...
Here is a `dcdnode` configuration example for the `trace_api_plugin` when tracing some DCD reference contracts:

```sh
dcdnode --data-dir data_dir --config-dir config_dir --trace-dir traces_dir
--plugin dcd::trace_api_plugin 
--trace-rpc-abi=dcd=abis/dcd.abi 
--trace-rpc-abi=dcd.token=abis/dcd.token.abi 
--trace-rpc-abi=dcd.msig=abis/dcd.msig.abi 
--trace-rpc-abi=dcd.wrap=abis/dcd.wrap.abi
```
...
The `trace-dir` option defines the directory on the filesystem where the trace log files are stored by the `trace_api_plugin`. These files are stable once the LIB block has progressed past a given slice and then can be deleted at any time to reclaim filesystem space. The deployed DCD system will tolerate any out-of-process management system that removes some or all of these files in this directory regardless of what data they represent, or whether there is a running `dcdnode` instance accessing them or not.  Data which would nominally be available, but is no longer so due to manual maintenance, will result in a HTTP 404 response from the appropriate API endpoint(s).
