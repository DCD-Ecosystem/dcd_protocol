...
Logging for `dcdnode` is controlled by the `logging.json` file. CLI options can be passed to `dcdnode` to [setup `logging.json`](10_setup-logging.json.md). The logging configuration file can be used to define [appenders](#appenders) and tie them to [loggers](#loggers) and [logging levels](20_logging-levels.md).

## Appenders

The logging library built into DCD supports two appender types:
...
The logging library built into DCD currently supports the following loggers:
