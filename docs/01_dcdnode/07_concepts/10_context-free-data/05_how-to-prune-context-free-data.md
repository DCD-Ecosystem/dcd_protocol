...
This how-to procedure showcases the steps to prune context-free data (CFD) from a transaction. The process involves launching the [`dcd-blocklog`](../../../10_utilities/dcd-blocklog.md) utility with the `--prune-transactions` option, the transaction ID(s) that contain(s) the context-free data, and additional options as specified below.

[[caution | Data Pruning on Public Chains]]
| Pruning transaction data is not suitable for public DCD blockchains, unless previously agreed upon through DCD consensus by a supermajority of producers. Even if a producing node on a public DCD network prunes context-free data from a transaction, only their node would be affected. The integrity of the blockchain would not be compromised.
...
* Review the [`dcd-blocklog`](../../../10_utilities/dcd-blocklog.md) command-line utility reference.
...
4. Launch the [`dcd-blocklog`](../../../10_utilities/dcd-blocklog.md) utility as follows:

   `dcd-blocklog [--blocks-dir <blocks_dir>] [--state-history-dir <state_hist_dir>] --prune-transactions --block-num <block_num> --transaction <trx_id> [--transaction <trx_id2> ...]`

   If the operation is *successful*:
   * The [`dcd-blocklog`](../../../10_utilities/dcd-blocklog.md) utility terminates silently with a zero error code (no error).
   ...
   * The [`dcd-blocklog`](../../../10_utilities/dcd-blocklog.md) utility outputs an error to `stderr` and terminates with a non-zero error code (indicating an error).

## Notes

Some additional considerations are in order:

* You can pass multiple transactions to [`dcd-blocklog`](../../../10_utilities/dcd-blocklog.md) if they are within the same block.
* You can use [`dcd-blocklog`](../../../10_utilities/dcd-blocklog.md) to display the block that contains the pruned transactions.
...
4. Launch the [`dcd-blocklog`](../../../10_utilities/dcd-blocklog.md) utility as follows:
   ...
   `dcd-blocklog --blocks-dir <blocks_dir> --state-history-dir <state_hist_dir> --prune-transactions --block-num 119 --transaction 1b9a9c53f9b692d3382bcc19c0c21eb22207e2f51a30fe88dabbb45376b6ff23`
