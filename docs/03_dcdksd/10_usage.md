---
content_title: dcdksd Usage
---

[[info | Recommended Usage]]
| For most users, the easiest way to use `dcdksd` is to have `dcdcli` launch it automatically. Wallet files will be created in the default directory (`~/dcd-wallet`).

## Launching dcdksd manually

`dcdksd` can be launched manually from the terminal by running:

```sh
dcdksd
```

By default, `dcdksd` creates the folder `~/dcd-wallet` and populates it with a basic `config.ini` file.  The location of the config file can be specified on the command line using the `--config-dir` argument.  The configuration file contains the HTTP server endpoint for incoming HTTP connections and other parameters for cross-origin resource sharing.
...
By default, `dcdksd` is set to lock your wallet after 15 minutes of inactivity. This is configurable in the `config.ini` by setting the timeout seconds in `unlock-timeout`. Setting it to 0 will cause `dcdksd` to always lock your wallet.

## Stopping dcdksd

The most effective way to stop `dcdksd` is to find the dcdksd process and send a SIGTERM signal to it.

## Other options

For a list of all commands known to `dcdksd`, simply run it with no arguments:
...
Config Options for dcd::http_plugin:
  --unix-socket-path arg (=dcdksd.sock)  The filename (relative to data-dir) to
...
Config Options for dcd::wallet_plugin:
