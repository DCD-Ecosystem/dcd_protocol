...
//                                    cpu_managed ? cpu : tot_itr->cpu_weight.amount );
      ...
      if ( stake_account != source_stake_from ) { //for dcd both transfer and refund make no sense
         ...
         if ( need_deferred_trx ) {
            ...
            dcd::cancel_deferred( from.value ); // TODO: Remove this line when replacing deferred trxs is fixed
            out.send( from.value, from, true );
         } else {
            dcd::cancel_deferred( from.value );
         }
