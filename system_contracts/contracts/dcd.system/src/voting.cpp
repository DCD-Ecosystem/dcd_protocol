...
namespace dcdsystem {
   ...
   void system_contract::regproducer( const name& producer, const dcd::public_key& producer_key, const std::string& url, uint16_t location ) {
   ...
   void system_contract::regproducer2( const name& producer, const dcd::block_signing_authority& producer_authority, const std::string& url, uint16_t location ) {
      require_auth( producer );
      check( url.size() < 512, "url too long" );
...
//      check( prod.confirmed != 1, "producer is confirmed" );
   ...
   bool system_contract::all_voted(const name& producer, const del_producer_vote& vote_info) {
      /* DCD_TODO
      auto active_prods = dcd::get_active_producers();
   ...
   void system_contract::update_elected_producers( const block_timestamp& block_time ) {
      ...
      using value_type = std::pair<dcd::producer_authority, uint16_t>;
...
//      for( auto it = idx.cbegin(); it != idx.cend() && 0 < it->confirmed && it->active(); ++it ) {
      ...
      std::vector<dcd::producer_authority> producers;
...
/*
      ...
      dcd_global_state gs = _global.get();
