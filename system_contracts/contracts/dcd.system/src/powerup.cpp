...
#include <dcd.system/powerup.results.hpp>
#include <algorithm>
#include <cmath>
...
void system_contract::adjust_resources(name payer, name account, symbol core_symbol, int64_t net_delta,
...

   {
      ...
      if (must_not_be_managed)
         dcd::check(!net_managed && !cpu_managed, "something is managed which shouldn't be");
...
void system_contract::cfgpowerup(powerup_config& args) {
   ...
   time_point_sec         now         = dcd::current_time_point();
   auto                   core_symbol = get_core_symbol();
   powerup_state_singleton state_sing{ get_self(), 0 };
   auto                   state = state_sing.get_or_default();

   dcd::check(dcd::is_account(reserv_account), "dcd.reserv account must first be created");
   ...
   auto is_default_asset = []( const dcd::asset& a ) -> bool {
   ...
   auto update = [&](auto& state, auto& args) {
      ...
      if (!args.assumed_stake_weight) {
         dcd::check(state.assumed_stake_weight != 0, "assumed_stake_weight does not have a default value");
      ...
      if (*args.current_weight_ratio == *args.target_weight_ratio) {
         *args.target_timestamp = now;
         ...
         if (!args.target_timestamp) {
            dcd::check(state.target_timestamp.utc_seconds != 0, "target_timestamp does not have a default value");
            *args.target_timestamp = state.target_timestamp;
         }
         dcd::check(*args.target_timestamp > now, "target_timestamp must be in the future");
      }
      ...
      if (!args.max_price) {
         dcd::check(!is_default_asset(state.max_price), "max_price does not have a default value");
      ...
      dcd::check(*args.current_weight_ratio > 0, "current_weight_ratio is too small");
      dcd::check(*args.current_weight_ratio <= powerup_frac, "current_weight_ratio is too large");
      dcd::check(*args.target_weight_ratio > 0, "target_weight_ratio is too small");
      dcd::check(*args.target_weight_ratio <= *args.current_weight_ratio, "weight can't grow over time");
      dcd::check(*args.assumed_stake_weight >= 1,
                   "assumed_stake_weight must be at least 1; a much larger value is recommended");
      dcd::check(*args.assumed_stake_weight * int128_t(powerup_frac) / *args.target_weight_ratio <=
                         std::numeric_limits<int64_t>::max(),
                   "assumed_stake_weight/target_weight_ratio is too large");
      dcd::check(*args.exponent >= 1.0, "exponent must be >= 1");
      dcd::check(*args.decay_secs >= 1, "decay_secs must be >= 1");
      dcd::check(args.max_price->symbol == core_symbol, "max_price doesn't match core symbol");
      dcd::check(args.max_price->amount > 0, "max_price must be positive");
      dcd::check(args.min_price->symbol == core_symbol, "min_price doesn't match core symbol");
      dcd::check(args.min_price->amount >= 0, "min_price must be non-negative");
      dcd::check(args.min_price->amount <= args.max_price->amount, "min_price cannot exceed max_price");
      if (*args.exponent == 1.0) {
         dcd::check(args.min_price->amount == args.max_price->amount, "min_price and max_price must be the same if the exponent is 1");
      }
   ...
   if (!args.min_powerup_fee) {
      dcd::check(!is_default_asset(state.min_powerup_fee), "min_powerup_fee does not have a default value");
      *args.min_powerup_fee = state.min_powerup_fee;
   }

   dcd::check(*args.powerup_days > 0, "powerup_days must be > 0");
   dcd::check(args.min_powerup_fee->symbol == core_symbol, "min_powerup_fee doesn't match core symbol");
   dcd::check(args.min_powerup_fee->amount > 0, "min_powerup_fee must be positive");
   ...
   update_weight(now, state.net, net_delta_available);
   update_weight(now, state.cpu, cpu_delta_available);
   dcd::check(state.net.weight >= state.net.utilization, "weight can't shrink below utilization");
   dcd::check(state.cpu.weight >= state.cpu.utilization, "weight can't shrink below utilization");
...
void system_contract::powerupexec(const name& user, uint16_t max) {
   ...
   dcd::check(state_sing.exists(), "powerup hasn't been initialized");
...
void system_contract::powerup(const name& payer, const name& receiver, uint32_t days, int64_t net_frac, int64_t cpu_frac,
   ...
   time_point_sec now         = dcd::current_time_point();
   auto           core_symbol = get_core_symbol();
   dcd::check(max_payment.symbol == core_symbol, "max_payment doesn't match core symbol");
   dcd::check(days == state.powerup_days, "days doesn't match configuration");
   dcd::check(net_frac >= 0, "net_frac can't be negative");
   dcd::check(cpu_frac >= 0, "cpu_frac can't be negative");
   dcd::check(net_frac <= powerup_frac, "net can't be more than 100%");
   dcd::check(cpu_frac <= powerup_frac, "cpu can't be more than 100%");

   int64_t net_delta_available = 0;
   int64_t cpu_delta_available = 0;
   process_powerup_queue(now, core_symbol, state, orders, 2, net_delta_available, cpu_delta_available);

   dcd::asset fee{ 0, core_symbol };
   auto         process = [&](int64_t frac, int64_t& amount, powerup_state_resource& state) {
      if (!frac)
         return;
      amount = int128_t(frac) * state.weight / powerup_frac;
      dcd::check(state.weight, "market doesn't have resources available");
      dcd::check(state.utilization + amount <= state.weight, "market doesn't have enough resources available");
      int64_t f = calc_powerup_fee(state, amount);
      dcd::check(f > 0, "calculated fee is below minimum; try powering up with more resources");
      fee.amount += f;
   ...
   if (fee > max_payment) {
      ...
      dcd::check(false, error_msg);
   }
   dcd::check(fee >= state.min_powerup_fee, "calculated fee is below minimum; try powering up with more resources");
   ...
   orders.emplace(payer, [&](auto& order) {
      ...
      order.expires    = now + dcd::days(days);
   });
   ...
   // inline noop action
   powup_results::powupresult_action powupresult_act{ reserv_account, std::vector<dcd::permission_level>{ } };
