...
namespace dcdsystem {
    ...
    _fee_proposals(get_self(), get_self().value)
   {
      ...
      _gstate3 = _global3.exists() ? _global3.get() : dcd_global_state3{};
      _gstate4 = _global4.exists() ? _global4.get() : get_default_inflation_parameters();
   }

   dcd_global_state system_contract::get_default_parameters() {
      dcd_global_state dp;
      get_blockchain_parameters(dp);
      return dp;
   }

   dcd_global_state4 system_contract::get_default_inflation_parameters() {
      dcd_global_state4 gs4;
   ...
   void system_contract::update_ram_supply() {
      auto cbt = dcd::current_block_time();
   ...
   void system_contract::setparams( const dcd::blockchain_parameters& params ) {
      require_auth( get_self() );
      (dcd::blockchain_parameters&)(_gstate) = params;
...
//   }
   ...
   void system_contract::activate( const dcd::checksum256& feature_digest ) {
      require_auth( get_self() );
...
//      set_resource_limits( newact, 0, 0, 0 );
   ...
   void native::setabi( const name& acnt, const std::vector<char>& abi ) {
      dcd::multi_index< "abihash"_n, abi_hash >  table(get_self(), get_self().value);
      auto itr = table.find( acnt.value );
      if( itr == table.end() ) {
         table.emplace( acnt, [&]( auto& row ) {
            row.owner = acnt;
            row.hash = dcd::sha256(const_cast<char*>(abi.data()), abi.size());
         });
      } else {
   ...
   void system_contract::init( unsigned_int version, const symbol& core ) {
      ...
      auto system_token_supply   = dcd::token::get_supply(token_account, core.code() );
