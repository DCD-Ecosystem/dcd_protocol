...
namespace dcd {
   ...
   /**
    * The `dcd.msig` system contract allows for creation of proposed transactions which require authorization from a list of accounts, approval of the proposed transactions by those accounts required to approve it, and finally, it also allows the execution of the approved transactions on the blockchain.
    *
    ...
    * - then you submit this proposal to the `dcd.msig` contract, and you also insert the account permissions required to approve this proposal into the command that submits the proposal to the blockchain,
    * - the proposal then gets stored on the blockchain by the `dcd.msig` contract, and is accessible for review and approval to those accounts required to approve it,
    * - after each of the appointed accounts required to approve the proposed transactions reviews and approves it, you can execute the proposed transaction. The `dcd.msig` contract will execute it automatically, but not before validating that the transaction has not expired, it is not cancelled, and it has been signed by all the permissions in the initial proposal's required permission list.
    */
   class [[dcd::contract("dcd.msig")]] multisig : public contract {
      public:
         ...
         void approve( name proposer, name proposal_name, permission_level level,
                       const dcd::binary_extension<dcd::checksum256>& proposal_hash );
         /**
         ...
         using propose_action = dcd::action_wrapper<"propose"_n, &multisig::propose>;
         using approve_action = dcd::action_wrapper<"approve"_n, &multisig::approve>;
         using unapprove_action = dcd::action_wrapper<"unapprove"_n, &multisig::unapprove>;
         using cancel_action = dcd::action_wrapper<"cancel"_n, &multisig::cancel>;
         using exec_action = dcd::action_wrapper<"exec"_n, &multisig::exec>;
         using invalidate_action = dcd::action_wrapper<"invalidate"_n, &multisig::invalidate>;

      private:
         struct [[dcd::table]] proposal {
            name                            proposal_name;
         ...
         typedef dcd::multi_index< "proposal"_n, proposal > proposals;

         struct [[dcd::table]] old_approvals_info {
            name                            proposal_name;
         ...
         typedef dcd::multi_index< "approvals"_n, old_approvals_info > old_approvals;
         ...
         struct [[dcd::table]] approvals_info {
            uint8_t                 version = 1;
            name                    proposal_name;
         ...
         typedef dcd::multi_index< "approvals2"_n, approvals_info > approvals;

         struct [[dcd::table]] invalidation {
            name         account;
         ...
         typedef dcd::multi_index< "invals"_n, invalidation > invalidations;
   };
} /// namespace dcd
