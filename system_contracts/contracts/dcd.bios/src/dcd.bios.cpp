...
void bios::setabi( name account, const std::vector<char>& abi ) {
   ...
   if( itr == table.end() ) {
      ...
      table.modify( itr, dcd::same_payer, [&]( auto& row ) {
         row.hash = dcd::sha256(const_cast<char*>(abi.data()), abi.size());
      });
   }
}
...
void bios::setprods( const std::vector<dcd::producer_authority>& schedule ) {
   require_auth( get_self() );
   set_proposed_producers( schedule );
}

void bios::setparams( const dcd::blockchain_parameters& params ) {
   require_auth( get_self() );
...
void bios::setkvparams( const dcd::kv_parameters& params ) {
   require_auth( get_self() );
...
void bios::activate( const dcd::checksum256& feature_digest ) {
   require_auth( get_self() );
   preactivate_feature( feature_digest );
}

void bios::reqactivated( const dcd::checksum256& feature_digest ) {
