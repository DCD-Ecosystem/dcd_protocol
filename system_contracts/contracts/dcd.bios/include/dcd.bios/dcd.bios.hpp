...
namespace dcdbios {
   ...
   /**
    * The `dcd.bios` is the first sample of system contract provided by `block.one` through the DCD platform. It is a minimalist system contract because it only supplies the actions that are absolutely critical to bootstrap a chain and nothing more. This allows for a chain agnostic approach to bootstrapping a chain.
    *
    * Just like in the `dcd.system` sample contract implementation, there are a few actions which are not implemented at the contract level (`newaccount`, `updateauth`, `deleteauth`, `linkauth`, `unlinkauth`, `canceldelay`, `onerror`, `setabi`, `setcode`), they are just declared in the contract so they will show in the contract's ABI and users will be able to push those actions to the chain via the account holding the `dcd.system` contract, but the implementation is at the DCD core level. They are referred to as DCD native actions.
    */
   class [[dcd::contract("dcd.bios")]] bios : public dcd::contract {
      public:
          ...
          */
         [[dcd::action]]
         void setprods( const std::vector<dcd::producer_authority>& schedule );
          ...
          */
         [[dcd::action]]
         void setkvparams( const dcd::kv_parameters& params );
          ...
          */
         [[dcd::action]]
         void reqactivated( const dcd::checksum256& feature_digest );

         struct [[dcd::table]] abi_hash {
            name              owner;
            checksum256       hash;
         ...
         typedef dcd::multi_index< "abihash"_n, abi_hash > abi_hash_table;
