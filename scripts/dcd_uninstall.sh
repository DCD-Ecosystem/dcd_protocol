...
function usage() {
  ...
  -i DIR      Directory where dcd is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the dcd data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export DCD_INSTALL_DIR=${INSTALL_LOCATION:-$DCD_INSTALL_DIR}
      ...
      case $PROCEED in
         ...
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/dcd")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/dcd")
            fi
            # Version < 1.8.0; Before we started using ~/dcd/1.8.x
            ...
            echo " - DCD Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled DCD Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing DCD Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/dcd_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
