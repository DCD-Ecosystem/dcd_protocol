...
( [[ $NAME == "Ubuntu" ]] && ( [[ "$(echo ${VERSION_ID})" == "16.04" ]] || [[ "$(echo ${VERSION_ID})" == "18.04" ]] || [[ "$(echo ${VERSION_ID})" == "20.04" ]])  ) || ( echo " - You must be running 16.04.x or 18.04.x or 20.04.x to install DCD." && exit 1 )

[[ $MEM_GIG -lt 7 ]] && echo "Your system must have 7 or more Gigabytes of physical memory installed." && exit 1
[[ "${DISK_AVAIL}" -lt "${DISK_MIN}" ]] && echo " - You must have at least ${DISK_MIN}GB of available storage to install DCD." && exit 1
...
ensure-apt-packages "${REPO_ROOT}/scripts/dcd_build_ubuntu_deps" $(echo ${EXTRA_DEPS[@]})
echo ""
