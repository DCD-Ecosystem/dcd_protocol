...
# the command line.
#
# In most cases, simply running ./dcd-tn_roll.sh is sufficient.
#

if [ -z "$DCD_HOME" ]; then
    echo DCD_HOME not set - $0 unable to proceed.
    exit -1
fi
...
if [ $DD = "all" ]; then
    for DCD_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $DCD_HOME/scripts/dcd-tn_up.sh "$*"
    done
else
    bash $DCD_HOME/scripts/dcd-tn_up.sh "$*"
fi
unset DCD_RESTART_DATA_DIR
